...
class Celeb
{
	...
	{
		$sql = "SELECT * FROM " . $this->table_cate . " WHERE user_id=".XT_USER_ID." AND parent_id='" . $this->db->escape($parent_id) . "'";
		if ($have_child) {
			$sql .= " AND id IN (SELECT DISTINCT parent_id FROM " . $this->table_cate . " WHERE parent_id > 0 AND user_id=".XT_USER_ID.") AND parent_id = '0'";
		}
	...
	{
		$sql = "SELECT COUNT(*) FROM " . $this->table_cate . " WHERE parent_id='" . $this->db->escape($parent_id) . "' AND user_id=".XT_USER_ID;
	...
	{
		$sql = "SELECT * FROM " . $this->table_cate . " WHERE parent_id='" . $this->db->escape($parent_id) . "' AND name='" . $this->db->escape($name) . "' AND user_id=".XT_USER_ID;
		if ($excep_id) {
	...
	function delCat($id)
	{
		$del_result = $this->db->delete($id, T_CELEB_CATEGORY,'id');
		if ($del_result) {
	...
	function getUserList($cid_1, $cid_2, $char_index = '', $nick = '', $offset = 0, $limit = '', $order = 'sort', $sort = 0)
	{
		...
		$sql = "SELECT * FROM " . $this->table_user . " WHERE user_id=".XT_USER_ID . $where_sql . " ORDER BY ".$order .",add_time DESC";
		if ($limit) {
			$sql .= ' LIMIT ' . (int)$offset . ',' . (int)$limit;
		}
	...
	function getUserNum($cid_1 = 0, $cid_2 = 0, $char_index = '', $nick = '')
	{
		...
		$sql = "SELECT COUNT(*) FROM " . $this->table_user . " WHERE user_id=".XT_USER_ID . $where_sql;
	...
	function getUserByInfo($data)
	{
		...
		$sql = "SELECT * FROM " . $this->table_user . " WHERE user_id=".XT_USER_ID . $where_sql . " ORDER BY sort ASC, add_time DESC";
		return RST($this->db->query($sql));
	}
	...
	function addStarUser($data)
	{
		...
		$data['user_id']=XT_USER_ID;
		$field_arr = array_keys($data);
		foreach($field_arr as $k => $field){
			$field_arr[$k] = '`'. $this->db->escape($field) .'`';
		}
		$value_arr = array_values($data);
		foreach($value_arr as $k => $value){
			$value_arr[$k] = $this->db->escape($value);
		}
	...
	function updateStarUser($data, $where)
	{
		...
		$set_sql = $where_sql = '';
		foreach ($data as $key => $value) {
			$set_sql .= ( '`'. $this->db->escape($key) . "`='" . $this->db->escape($value) . "',");
		}
		$set_sql = rtrim($set_sql, ',');
		
		foreach ($where as $key => $value) {
			$where_sql .= (($where_sql ? ' AND ' : " ") . '`'. $this->db->escape($key) . "`='" . $this->db->escape($value) . "'");
		}
		
		$sql = "UPDATE " . $this->table_user . " SET " . $set_sql . " WHERE user_id=".XT_USER_ID .' AND' . $where_sql;
	...
	{
		$sql = "DELETE FROM " . $this->table_user . " WHERE c_id1='" . $this->db->escape($c_id1) . "' AND c_id2='" . $this->db->escape($c_id2) . "' AND sina_uid='" . $this->db->escape($sina_uid) . "' AND user_id=".XT_USER_ID;

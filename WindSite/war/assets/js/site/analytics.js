function initAdvanced() {
	...
	$('.analytics').click(function() {
		...
		if ('source' == name) {
			...
			advancedAnalytics(PID, 1, '-ga:date');
		}
		return false;
	});
 ...
 * 高级统计
 */
function advancedAnalytics(pid, pageNo, sort, filter) {
	var dimensions = [];
	var categoryFilter = "";
	if (!pageNo) {
		pageNo = 1;
	...
	getAdvancedAnalytics(pid, pageNo, startDate, endDate, dimensions,
			categoryFilter, sort)
}
function getAdvancedAnalytics(pid, pageNo, startDate, endDate, dimensions,
		categoryFilter, sort) {
	$('#tableProfile')
			.empty()
			.append("<tr id='loading'><td><div  align='left'>正在加载数据,请稍候...</div></td></tr>");
	$.ajax({
				url : "/router/member/ga/advanced/" + pid + '?v='
						+ Math.random(),
				type : 'GET',
				data : {
					pageNo : pageNo,
...
function convertCategoryFilterValue(action) {
	switch (action) {
		...
		case 'channel' :
			return $('#category-channels').val();
		case 'blog' :
			return $('#category-blogs').val();
		case 'activity' :
			return $('#category-activitys').val();
	}
}
/**
 * 创建统计图形报表
 ...
 *            pid
 */
function createSiteProfile(content, pid) {
	createAnalytics(content, '/router/member/ga/siteprofile/' + pid, 'GET', {},
			function(data) {
				if (data.length == 0) {
 ...
 *            date
 */
function createHourProfile(content, pid, date) {
	$('#profileBody').empty();
	createAnalytics(content, '/router/member/ga/hour/' + pid, 'GET', {
				date : date
			}, function(data) {
				if (data.length == 0) {
					alert('未查询到时段分析数据');
 ...
 *            endDate
 */
function createDayProfile(content, pid, startDate, endDate, year, month) {
	$('#profileBody').empty();
	createAnalytics(content, '/router/member/ga/day/' + pid, 'GET', {
				startDate : startDate,
				endDate : endDate,
				year : year,
				month : month
 ...
 *            pid
 */
function createLastVisitProfile(content, pid) {
	$("#" + content).empty();
	$("#" + content)
			.append("<tr id='loading'><td colspan=5><div align='left'>正在加载数据,请稍候...</div></td></tr>");
	var sender = new WindSender('/router/member/ga/lastvisit/' + pid + '?v='
			+ Math.random());
...
function convertAnalyticsLabel(action, label) {
	...
	if (action.indexOf('item-') != -1)
		return '<a style="color:#00e;" href="/router/member/analyticsmanager/advanced?dimensions=date,category,label,city&categoryFilter=item:'
				+ as[as.length - 1] + '">' + label + '</a>';
	else if (action.indexOf('shop-') != -1)
		return '<a style="color:#00e;" href="/router/member/analyticsmanager/advanced?dimensions=date,category,label,city&categoryFilter=shop:'
				+ as[as.length - 1] + '">' + label + '</a>';
	else if (action.indexOf('channel-') != -1)
		return '<a style="color:#00e;" href="/router/member/analyticsmanager/advanced?dimensions=date,category,label,city&categoryFilter=channel:'
				+ as[as.length - 1] + '">' + label + '</a>';
	else if (action.indexOf('key-') != -1)
		return '<a style="color:#00e;" href="/router/member/analyticsmanager/advanced?dimensions=date,category,label,city&categoryFilter=key:'
				+ as[as.length - 1] + '">' + label + '</a>';
	else if (action.indexOf('blog-') != -1)
		return '<a style="color:#00e;" href="/router/member/analyticsmanager/advanced?dimensions=date,category,label,city&categoryFilter=blog:'
				+ as[as.length - 1] + '">' + label + '</a>';
	else if (action.indexOf('activity-') != -1)
		return '<a style="color:#00e;" href="/router/member/analyticsmanager/advanced?dimensions=date,category,label,city&categoryFilter=activity:'
				+ as[as.length - 1] + '">' + label + '</a>';
	else
		return label;
}

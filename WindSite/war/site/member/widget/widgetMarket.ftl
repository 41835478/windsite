...
<body>
<div class="comment_star" id="comment_star_div" style="position: absolute;width:192px;display:none;padding:15px;" align="center"></div>
			...
			<div><a href="/"><img src="/assets/images/logo.png"></a></div></td><td valign=bottom>
			  <div class="wm_mac_nav" align=center>
			     <ul style="width:500px">
			      ...
			      <li><a href="/router/member/widget/market" class="current">组件超市<small>(${page.totalCount})</small></a></li>
...
<input type="hidden" id="sortOrderHidden" value="${sortOrder}">
	<div class="search_sort_div">
      <ul class="search_sort_ul">
        <li	<#if sortOrder=="widgetUpdated_desc">class="current"</#if>><a href="/router/member/widget/market?q=${q}&pageNo=${page.pageNo}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=widgetUpdated_desc">更新时间排行</a></li>
        <li	<#if sortOrder=="favorite_desc">class="current"</#if>><a href="/router/member/widget/market?q=${q}&pageNo=${page.pageNo}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=favorite_desc"><i class="ssu_pj_i"></i>收藏排行</a></li>
        <li	<#if sortOrder=="used_desc">class="current"</#if>><a href="/router/member/widget/market?q=${q}&pageNo=${page.pageNo}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=used_desc"><i class="ssu_rq_i"></i>使用排行</a></li>
      </ul>
      <div class="wm_search">
      ...
      <div class="clearing"></div>			
			...
			<div class="sort_pagina">
		    	<a href="<#if (page.pageNo<page.totalPageCount)>/router/member/widget/market?q=${q}&pageNo=${page.pageNo+1}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=${sortOrder}<#else>#</#if>" class="page_next <#if (page.pageNo>=page.totalPageCount)>page_next_no</#if>"></a>
    			<span class="pagina_span">${page.pageNo}/${page.totalPageCount}</span>
				<a href="<#if (page.pageNo>1)>/router/member/widget/market?q=${q}&pageNo=${page.pageNo-1}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=${sortOrder}<#else>#</#if>" class="page_pre <#if !(page.pageNo>1)>page_pre_no</#if>"></a>
        	</div>
          ...
          适合投放：<#if w.cat??><a href="/router/member/widget/market?cid=${w.cat.cid}"><span class="k">${w.cat.name}</span></a></#if>　  　 
            布局:<a href="/router/member/widget/market?layout=${w.layout}"><span class="k"><@ws.layout w.layout></@ws.layout></span></a>　  
            作者：<a href="/router/member/widget/designers/${w.createdBy}">${w.nick}</a>
				...
				<#if layoutfilter=='-1'>
				超市中还没有当前搜索条件下的组件，到<a href="/router/member/widget/sys" style="font-size:14px;">组件模板</a>中挑选一个自己设计。
				<#else>
				超市中还没有【<strong style="color:#F60;font-size:14px;"><@ws.layout layoutfilter></@ws.layout></strong>】布局下的组件，到<a href="/router/member/widget/sys?layout=${layoutfilter}" style="font-size:14px;">组件模板</a>中挑选一个自己设计。
				</#if>
			</#if>
			<#assign url='/router/member/widget/market?q='+q+'&layout='+layoutfilter+'&cid='+cidfilter+'&sortOrder='+sortOrder+'&type='+typefilter>

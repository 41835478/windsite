...
public class FanliClient extends PHPFunctions {
	...
	public String uc_api_post(String $module, String $action,
		...
		String $postdata = uc_api_requestdata($module, $action, $s.toString(),
				"");
		return uc_fopen2(getUc_api() + "/index.php", 500000, $postdata, "",
				true, UC_IP, 20, true);
	}
	...
	protected String uc_api_url(String $module, String $action, String $arg,
			String $extra) {
		String $url = getUc_api() + "/index.php?"
	 ...
	 * @return
	 */
	public String uc_pm_send(Integer fromuid, String msgto, String subject,
			String message, Boolean instantly, Integer replypid,
			Boolean isusername) {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("fromuid", fromuid);
		args.put("msgto", msgto);
		args.put("subject", subject);
		args.put("message", message);
		args.put("instantly", instantly ? 1 : 0);
		args.put("replypid", replypid);
		args.put("isusername", isusername ? 1 : 0);
		return call_pm_func(UC_API_FUNC, "pm", "sendpm", args);
	}
	 ...
	 * 
	 * @param icon
	 *            图标类型，如：thread、post、video、goods、reward、debate、blog、album、comment
	 *            、wall、friend
	 * @param uid
	 *            用户 ID
	 * @param username
	 ...
	 *            保留
	 * @param images
	 *            相关图片的 URL 和链接地址，格式请参考实例
	 * @return
	 */
	public String uc_feed_add(String icon, Integer uid, String username,
			String title_template, Map<String, Object> title_data,
			String body_template, Map<String, Object> body_data,
			String body_general, String target_ids, String[] images) {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("icon", icon);
		args.put("uid", uid);
		args.put("username", username);
		args.put("title_template", title_template);
		args.put("title_data", title_data);
		args.put("body_template", body_template);
		args.put("body_data", body_data);
		args.put("body_general", body_general);
		args.put("target_ids", target_ids);
		args.put("images", images);
		return call_pm_func(UC_API_FUNC, "feed", "add", args);
	}
	 ...
	 * @return
	 */
	public String uc_friend_add(Integer uid, Integer friendid, String comment) {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("uid", uid);
		args.put("friendid", friendid);
		args.put("comment", comment);
		return call_pm_func(UC_API_FUNC, "friend", "add", args);
	}
	...
	public String getUc_api() {
		if (StringUtils.isNotEmpty(uc_api)) {
			return uc_api;
		}
		return UC_API;
	}

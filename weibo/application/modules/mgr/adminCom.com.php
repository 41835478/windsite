...
class adminCom {
	...
	function getAdminsByGroupAndAutoFans() {
		$db = APP :: ADP('db');
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_ADMIN . ' WHERE `sina_uid` is not null AND `sina_uid` !=\'\' AND `v2_access_token` !=\'\' AND `group_id`>4 ORDER BY `id`';
		return RST($db->query($sql));
	}
	/**
	 * 查询已绑定新浪微博的管理员ID列表
	...
	function getAdminsSinaUidByAutoFans(){
		$db = APP :: ADP('db');
		$sql = 'SELECT distinct(sina_uid) FROM ' . $db->getPrefix() . T_ADMIN . ' WHERE `sina_uid` is not null AND `sina_uid` !=\'\' AND `v2_access_token` is not null AND `v2_access_token` !=\'\' ORDER BY `id`';
		return RST($db->query($sql));
	}
	/**
	 * 查询已绑定新浪微博的管理员列表
	...
	function getAdminsByAutoFans(){
		$db = APP :: ADP('db');
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_ADMIN . ' WHERE `sina_uid` is not null AND `sina_uid` !=\'\' AND `v2_access_token` is not null AND `v2_access_token` !=\'\' ORDER BY `id`';
		return RST($db->query($sql));
	}
	/**

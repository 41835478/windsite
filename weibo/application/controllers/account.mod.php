...
class account_mod {
	 ...
	 */
	function qqLogin() {
		//$this->gloCheckLogin(); //TODO 是否检查新浪微博是否登录
		$isPop = V('g:popup');
		$loginCallBack = V('g:loginCallBack', '');

		OpenSDK_Tencent_Weibo :: init(WB_QQ_AKEY, WB_QQ_SKEY);
		$request_token = OpenSDK_Tencent_Weibo :: getRequestToken($loginCallBack);
	...
	function taobaoLogin() {
		$this->gloCheckLogin(); //检查新浪微博是否登录
		$loginCallBack = V('g:loginCallBack', '');
		$sina_uid = USER :: uid();
		$refresh = urlencode('http://' . $_SERVER['SERVER_NAME'] . '/account.refresh?' . 'redirect=' . urlencode($loginCallBack));
		APP :: redirect(TB_CONTAINER . ('&site=' . XT_USER_ID) . ('&sina=' . $sina_uid) . ('&refresh=' . ($refresh)), 3);
	}
	//淘宝回调
	function tbCallback() {
		$site = V('g:site', '');
		$sina = V('g:sina', '');
		if (empty ($site) || empty ($sina)) {
			echo '当前请求不正确';
			exit;
		}
		...
		$r = DR('mgr/userCom.insertUserByUserId', '', array (
			'taobao_uid' => $ret['userId'],
			'taobao_nick' => $ret['nick'],
			'taobao_session' => $ret['session']
		), $sina, $site);
		//TODO 是否需要此刻初始化淘宝帐号其他内容（可考虑之后在相关功能处在初始化）
		$goUrl = V('g:refresh', 'http://' . $_SERVER['SERVER_NAME'] . '/account.refresh');
		$loadUrl = $goUrl ? $goUrl : URL('index');
		APP :: redirect($loadUrl, 3);
		exit;
	}
	//从其他平台登录进入，重新初始化平台相关SESSION
	function refresh() {
		$this->gloCheckLogin(); //检查新浪微博是否登录
		$r = DR('mgr/userCom.getByUid', '', USER :: uid());
		if ($r['rst']) { //初始化平台级数据
			$new = $r['rst'];
			USER :: taobaoId($new['taobao_uid']);
			USER :: set('taobao_session', $new['taobao_session']);
			USER :: set('taobao_nick', $new['taobao_nick']);
			USER :: set('extra', $new);
		}
		$goUrl = V('g:redirect', false);
		$loadUrl = $goUrl ? $goUrl : URL('index');
		APP :: redirect($loadUrl, 3);
		exit;
	}
	...
	function _goLogin() {
		...
		$q = V('g', array ());
		if (isset ($q[R_GET_VAR_NAME])) {
			unset ($q[R_GET_VAR_NAME]);
		}
	...
	function _setSinaLoginSession($token, $user = null) {
		...
		$userRst = $user_info['rst'];
		USER :: taobaoId($userRst['taobao_uid']);
		USER :: set('taobao_session', $userRst['taobao_session']);
		USER :: set('taobao_nick', $userRst['taobao_nick']);
		USER :: set('extra', $userRst);
		// 设置个性域名

...
$(function() {
	...
	$('#widget-customer').load('/router/member/designer/blogdesigner/widget/${widget.id}',function(){
	});
	<#elseif 'update'==designer&&cwidget??>
		$('#widget_cats').val('${cwidget.cat.cid}');
		$('#classes').val('${cwidget.classid}');
		$('#bloglength').val('${cwidget.bloglength}');
		$('input[type="radio"][name="friendRadio"][value="${cwidget.friend}"]').attr('checked',true);
		loadBlogCustomeWidget('${widget.id}','${cwidget.classid}','${cwidget.bloglength}');
	</#if>
});
</script>
<style>
...
<@ws.info>
	...
	<br/>
		...
		1.选择家园日志分类<br/>
		2.选择日志显示数量<br/>
		备注:家园日志组件完成制作后。以后您在新淘家园的当前分类下发布日志，会自动更新该文章列表和使用了该文章列表的页面
	</span>
...
<tr><td>
<select id="classes">
<option value="0">请选择家园日志分类</option>
<#list classes as c><option value="${c.classid}">${c.classname}</option></#list>
</select>
</td><td><select id="bloglength"><option value="0">选择日志显示数量</option><option value="5">5</option><option value="6">6</option><option value="7">7</option><option value="8">8</option><option value="9">9</option><option value="10">10</option></select></td></tr>
<#else><tr><td>您尚未在新淘网家园建立日志分类，进入<a href="/router/site/loginuc?redirect=http://home.xintaonet.com">新淘家园</a>创建日志分类</td></tr>
</#if>
</table>
</td></tr>
<tr>

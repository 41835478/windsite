 ...
 * 
 * @param eName checkbox的元素名称 @param isCheck true:选择 false:取消选择
 * 
 * @example: <input name="albumID[]" type="checkbox" value="1" />
 * 
 * <input name="albumIDCheckBtn" onclick="checkAll('albumID[]', this.checked)"
 * type="checkbox" />
 */
...
function isID(str) {
	...
	return ((isInt(str) && str > 0) || reg1.test(str) || reg2.test(str) || reg3
			.test(str));
}

/**
 * 检查元素是否在数组中 返回true or false
 */
...
function isLessNow(str) {
	...
	if (result[1].length < 4) {
		if (new Number(result[1]) > (d.getFullYear() % 100)
				&& new Number(result[1]) >= 70) {
...
/**
 * 功能: 验证表单中,用户输入内容是否适合 form 表单对象 showmode 显示方式：'alert'、'div'(默认) alert:
 * 使用alert函数提示 div: 显示在"元素名_msg"的DIV或TD
 */
function checkForm(form, showmode, act) {
	...
	try {
		CHECKELE : for (var i = 0; i < eles.length; i++) {
			var unitresult = true;
			var ele = eles[i];
				...
				for (vs = 0; vs < valids.length; vs++) {
					if (valids[vs].indexOf(':') > 0) {
						...
						if (param[0] == 'type' && ele.value != '') {
							...
							result = in_array(ftype[ftype.length - 1]
											.toLowerCase(), types);
							if (!result) {
			...
			if (ele.type == 'text' || ele.type == 'textarea'
					|| ele.type == 'password' || ele.type == 'hidden') {
				for (var j = 0; j < valids.length; j++) {
					...
					switch (valids[j]) {
						...
						case 'date' :
							...
							// 只有完整的日期格式(年、月、日写全)才执行测试
							if (in_array("more_than_now", valids)
									&& eleValue
											.match(/\d{2,4}[\/\-]\d{1,2}[\/\-]\d{1,2}/)
									&& !isMoreThanNow(eleValue)) {
								errInfo += "不能少于当前日期";
								unitresult = false;
							}
							break;
...
function tigMsgConfirm(msg) {
	...
	top = Math.floor(bodyheight / 2) - Math.floor(height / 2)
	...
	index : {
		...
		selectMainMenu : function(n, m) {
			with (admin.index) {
				...
				if (!$selectedMainMenu.length
						&& !($selectedMainMenu = $mainmenu.eq(0)).length) {
					$selectedMainMenu = null;
					return;
				}
				...
				$selectedSubMenuContainer = $submenu_container.eq($mainmenu
						.index($selectedMainMenu));
				selectSubMenu(m);
		...
		selectSubMenu : function(n, jump) {
			...
			with (admin.index) {
				...
				if (!$selectedSubMenuContainer.length
						&& !($selectedSubMenuContainer = $submenu_container
								.eq(0)).length) {
					$selectedSubMenuContainer = null;
					return;
				}
				...
				if (!$selectedSubMenu.length
						&& !($selectedSubMenu = $submenu.eq(0)).length) {
					$selectedSubMenu = null;
					return false;
				}
				$selectedSubMenu.parent('li').addClass('current')
						.trigger('blur');
				if (jump) {
					// 如果该菜单无访问权限，则不跳转并弹出升级框，fixed by fxy060608
					if ('true' == $selectedSubMenu.attr('data-access')) {
						// 判断该菜单功能是否需要登录新浪微博
						if ('true' == $selectedSubMenu.attr('data-sina')) {
							Xwb.ui.MsgBox.error('提示', '您需要绑定并登录新浪微博后才可以使用该功能！',
									function() {
										top.location.href = "/admin.php?m=mgr/admin.index#1,5";
										top.location.reload();
									}, 'close');
						} else {
							var router = isElement(n) ? '&router='
									+ $(n).attr('router') : '';
							$('#mainframe').attr('src',
									$selectedSubMenu.attr('href') + router);
						}
					} else {
						// 显示演示图片
						var m = $selectedSubMenu.attr('href').split('mgr/')[1];
						$('#mainframe').attr('src',
								'/admin.php?m=mgr/xintao/xintao.demo&module=' + m);
//						Xwb.ui.MsgBox.tipWarn('您需要升级版本之后才可以使用该功能！<a>立刻升级</a>',
//								function() {
//								});
					}
		...
		init : function() {
			with (admin.index) {
				...
				$mainmenu_container.click(function(e) {
							var self = this;
							mainframeUnload(function() {
										selectMainMenu.call(admin.index, self);
									});
							return false;
						});
				$submenu_container.click(function(e) {
							if (e.target.tagName.toLowerCase() == 'a'&&$(e.target).hasClass('a-menu')) {
								console.log(e.target);
								mainframeUnload(function() {
											selectSubMenu.call(admin.index,
													e.target);
										});
								return false;
							}
...
function compareVersion(ver1, ver2) {
	...
	var maxLen = arr1.length == arr2.length ? arr1.length : Math.max(
	...
	$.getJSON(url + '&jsonp=?', function(r) {
		...
		if (compareVersion(r.ver, currVer) > 0) {
			var html = [
					...
					r.text,
					'<a id="up_yes" class="update-link" href="'
							+ r.downurl
							+ '" target="_blank">立即升级</a><a href="javascript:;" id="up_no">以后再说</a>',
					'</div></div>'].join('');

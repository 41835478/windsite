...
?>
									...
									<div class="feed-list <?php if (!in_array($header, array(1,2,3))){echo 'mblog-list';}?>">
										...
										<div class="title-box">
											...
											<?php else:?>
											<a rel="nofollow" href="<?php echo URL(APP::getRequestRoute(), 'filter_type=1'. $filter_uid);?>"><?php LO('modules__weiboList__ori');?></a>|
											<?php endif;?>
											<?php if ($filter_type == 2):?>
												<strong><?php LO('modules__weiboList__image');?></strong>|
											<?php else:?>
											<a rel="nofollow" href="<?php echo URL(APP::getRequestRoute(), 'filter_type=2'. $filter_uid);?>"><?php LO('modules__weiboList__image');?></a>| 
											<?php endif;?>
											<?php if ($filter_type == 3):?>
												<strong><?php LO('modules__weiboList__video');?></strong>|
											<?php else:?>
											<a rel="nofollow" href="<?php echo URL(APP::getRequestRoute(), 'filter_type=3'. $filter_uid);?>"><?php LO('modules__weiboList__video');?></a>|
											<?php endif;?>
											<?php if ($filter_type == 4):?>
												<strong><?php LO('modules__weiboList__music');?></strong>
											<?php else:?>
											<a rel="nofollow" href="<?php echo URL(APP::getRequestRoute(), '&filter_type=4' . $filter_uid);?>"><?php LO('modules__weiboList__music');?></a>
											<?php endif;?>
											</div>
											<?php endif;?>
											<?php }?>

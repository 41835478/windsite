...
<title>每日分析-推广统计-我是淘客-新淘网</title>
...
<script>
$(function(){
	createDayProfile('chartProfile','${USER.pid}','${startThisWeekDate}','${endThisWeekDate}');
	$('input[type="radio"][name="date"]').change(function(){
		switch($(this).val()){
			...
			break;
			case 'lastWeek':
			createDayProfile('chartProfile','${USER.pid}','${startLastWeekDate}','${endLastWeekDate}');
			break;
			case 'last30':
			createDayProfile('chartProfile','${USER.pid}','${startMonthDate}','${endMonthDate}');
			break;
		}
	});
	$('#subAnalytics').click(function(){
		createDayProfile('chartProfile','${USER.pid}','','',$('#dateYear').val(),$('#dateMonth').val());
	});
});
</script>
<@xt.taoketemplate navselected='taoke' bdselected='analytics-day' group=4>

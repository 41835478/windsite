...
class events
{
	...
	function eventSearch($keyword = '', $state = 0, $sina_uid = 0, $join = false, $offset = 0, $limit = 50)
	{
		...
		$where_sql = ' AND user_id=' . XT_USER_ID . (!empty($where) ? (' AND ' . implode(' AND ', $where)) : '');
	...
	function getEventCount($state = 0, $sina_uid = false) {
		...
		$where_sql = !empty($where) ? (' AND user_id=' . XT_USER_ID . ' AND ' . implode(' AND ', $where)) : '';
	...
	function getMineAttendEvents($uid, $page = 1, $limit = 50) {
		...
		$sql = 'SELECT e.id, e.title, e.addr, e.desc, e.cost, e.sina_uid, e.nickname, e.realname, e.phone, e.start_time, e.end_time, e.pic, e.wb_id, e.join_num, e.state, e.other FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' AS j LEFT JOIN ' . $this->table . ' AS e ON j.event_id = e.id and j.user_id = e.user_id WHERE j.sina_uid = "' . $this->db->escape($uid) . '" AND j.user_id=' . XT_USER_ID .' ORDER BY e.add_time DESC' . $limit_str;

		$data = $this->db->query($sql);
		for ($i=0, $count=count($data); $i<$count; $i++) {
			$data[$i]['state_num'] = $this->getState($data[$i]);
		}
		$this->count_sql = 'SELECT COUNT(*) FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' AS j LEFT JOIN ' . $this->table . ' AS e ON j.event_id = e.id AND j.user_id=e.user_id WHERE j.sina_uid = "' . $this->db->escape($uid) . '" AND j.user_id='.XT_USER_ID;
		return RST($data);
	}
	...
	function getMineAttendEventsCount($uid) {
		$sql = 'SELECT COUNT(*) FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' AS j LEFT JOIN ' . $this->table . ' AS e ON j.event_id = e.id AND j.user_id=e.user_id WHERE j.sina_uid = "' . $this->db->escape($uid) . '" AND j.user_id='.XT_USER_ID;
		return RST($this->db->getOne($sql));
	}

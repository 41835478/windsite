...
class userRecommendCom {
	...
	*/
	function addRelatedId($group_id, $id, $type = 1) {
		...
		$db->setTable(T_COMPONENT_USERGROUPS);
		$row = "";
		$sql = 'SELECT related_id FROM ' . $db->getPrefix() . T_COMPONENT_USERGROUPS . ' WHERE `group_id`=' . $group_id . ' AND `user_id`='.XT_USER_ID;
		$row = $db->getRow($sql);
...
//		}
		...
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_COMPONENT_USERS . ' WHERE `group_id` = ' . $keyword . ' AND `user_id`='.XT_USER_ID.' ORDER BY `sort_num` DESC';
		$rs = $db->getRow($sql);
	...
	*/
	function delByUid($uid, $group_id) {
		...
		$db = APP :: ADP('db');
		$sql = 'DELETE FROM ' . $db->getPrefix() . T_COMPONENT_USERS . ' WHERE `uid` = ' . $uid . ' AND `group_id` = ' . $group_id . ' AND `user_id`='.XT_USER_ID;
	...
	*/
	function delUserByUid($uid) {
		...
		$this->_cleanCache();
		$db = APP :: ADP('db');
		$sql = 'DELETE FROM ' . $db->getPrefix() . T_COMPONENT_USERS . ' WHERE `uid` = ' . $uid . ' AND `user_id`='.XT_USER_ID;
		return RST($db->execute($sql));
	}
	...
	*/
	function delAllByUid($uids, $group_id) {
		...
		$this->cleanRelated($group_id);
		$db = APP :: ADP('db');
		$sql = 'DELETE FROM ' . $db->getPrefix() . T_COMPONENT_USERS . ' WHERE `group_id` = ' . $group_id . ' AND `uid` IN (' . $uids . ')' . ' AND `user_id`='.XT_USER_ID;
		return RST($db->execute($sql));
	}
	...
	*/
	function userSortById($uid, $group_id, $sort_num) {
		...
		$this->_cleanCache();
		$db = APP :: ADP('db');
		$sql = 'UPDATE ' . $db->getPrefix() . T_COMPONENT_USERS . ' SET sort_num = ' . $sort_num . ' WHERE `group_id` = ' . $group_id . ' AND `uid` = ' . $uid . ' AND `user_id`='.XT_USER_ID;
		return RST($db->execute($sql));
	}
	...
	*/
	function getUserByUid($group_id, $uid) {
		...
		$db = APP :: ADP('db');
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_COMPONENT_USERS . ' WHERE `group_id` = ' . $group_id . ' AND `uid` = ' . $uid . ' AND `user_id`='.XT_USER_ID;
	...
	*/
	function updateUser($data, $uid, $group_id) {
		...
		$this->cleanRelated($group_id);
		$db = APP :: ADP('db');
		$keyword = $uid . ' AND `group_id`=' . $group_id . ' AND `user_id`='.XT_USER_ID;
		$db->setTable(T_COMPONENT_USERS);

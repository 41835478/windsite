...
APP :: addPreDoAction('apiStop.check', 'c', false, array (
	'welcome.retry'
)); // 检查API是否中
APP :: addPreDoAction('account.initSiteInfo', 'm', false, array (
	'pipe.t'
));
APP :: addPreDoAction('account.allowedLogin', 'm', false, array (
	'account.logout'
));
if (USER :: sys('sysLoginModel')) {
	APP :: addPreDoAction('account.gloCheckLogin', 'm', false, array (
		'welcome.retry',
		'pipe.t',
		'authImage.paint',
		'account.*',
...
//校验权限
APP :: addPreDoAction('xintao.check_versionno', 'f', array (), false);

/// 初始化应用程序
APP :: init();

define('TB_APP_KEY', TB_LOGIN_APP_KEY);
define('TB_APP_SECRET', TB_LOGIN_APP_SECRET);
...
if (!in_array(APP :: getRequestRoute(), array (
		...
		'go.default_action',
		'go'
	)) && (XT_IS_WEIBO == 'false' || (XT_FREE_DATELINE != '' && time() > strtotime(XT_FREE_DATELINE)))) { //升级服务不屏蔽
	F('xintao.xintao_error', '需订购增值服务');
}
if (!in_array(APP :: getRequestRoute(), array (
		'map.upgrade',
		'go.default_action',

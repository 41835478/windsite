...
//	}
	...
	$profile_url = URL('ta', array('id' => (string)$user['id']));
...
?>
		...
		<div class="box-style">
			...
			<div class="forward box-content">
				<p><a href="<?php echo URL('ta',array('id' => $rtUser['id']));?>"><?php echo $rtNick;?><?php echo F('verified', $rtUser);?></a>：<?php echo $rtText;?>
					<span><a href="<?php echo $rtLink;?>" id="lk_fw" rel="nofollow"><?php LO('modules__feed__retRepost');?></a>|<a href="<?php echo $rtLink;?>" id="lk_cm" rel="nofollow"><?php LO('modules__feed__retComment');?></a></span>
				</p>
	...
	</p><span><a href="<?php if (isset($xwb_weibo_verify)):?>#<?php else:?><?php echo $link;?><?php endif;?>" rel="nofollow" ><?php echo $format_time;?></a> <?php if (isset($xwb_weibo_verify)) :?><?php LO('modules__feed__informationIsNotAudited');?><?php else:?><?php LO('modules__feed__source', $source);?><?php endif;?></span>
    ...
    <?php if(isset($item)&&!empty($item)){
    	$rel = ' class="J_TrackItem No" data-nid="'.$item['num_iid'].'" data-nick="'.$item['nick'].'" ';
    	echo '<div class="feed-ads"><a target="_blank" '.$rel.' href="/item/id-'.$item['num_iid'].'">'.$item['title'].'</a> ￥'.$item['price'].'</div>';
    }?>
  </div>
<?php
//if (!isset($xwb_weibo_verify)) {

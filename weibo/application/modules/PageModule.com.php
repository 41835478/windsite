...
class PageModule {
	...
	function getQuotePages($component_id) {
		$sql = 'SELECT pm.page_id,p.page_name' . ' FROM xwb_page_manager pm' . ' LEFT JOIN xwb_pages p' . ' ON pm.page_id = p.page_id' . ' WHERE component_id=' . (int) $component_id . ' AND in_use = 1 AND user_id=' . XT_USER_ID;
	...
	function getPage($page_id) {
		$db = $this->db;
		$com = $db->getRow('select * from ' . $db->getTable(T_PAGES) . ' where page_id=' . (int) $page_id . ' and user_id=' . XT_USER_ID);
		return RST($com);
	}
	...
	function config($cfgName = null, $cfgValue = null, $componentId = NULL) {
		...
		if (is_null($cfgValue)) {
			$rs = $db->query('select * from ' . $db->getTable(T_COMPONENTS_CFG) . ' where component_id=' . (int) $this->component_id . ' and cfgName=\'' . $db->escape($cfgName) . '\' and user_id=' . XT_USER_ID);

			return RST($rs);
		} else {
			$rs = $db->execute('update ' . $db->getTable(T_COMPONENTS_CFG) . ' set cfgValue=\'' . $db->escape($cfgValue) . '\' where cfgName=\'' . $db->escape($cfgName) . '\' and component_id=' . $this->component_id . ' and user_id=' . XT_USER_ID);
	...
	function configList($useRstFormat = false, $component_id = null, $forceResetCache = false) {
		...
		if ($forceResetCache || ($rs = CACHE :: get($cache_key)) === false) {
			$rs = $this->db->query('select * from ' . $this->db->getTable(T_COMPONENTS_CFG) . ' where component_id=' . $component_id . ' and user_id=' . XT_USER_ID);
	...
	function getPageModules($page_id = null, $group = false) {
		$db = $this->db;
		$sql = 'select position,isNative,param,p.id,p.title as newTitle,p.sort_num,p.in_use,c.* from ' . $db->getTable(T_PAGE_MANAGER) . ' as p' . ' join ' . $db->getTable(T_COMPONENTS) . ' as c' . ' on c.component_id=p.component_id' . ' where page_id=' . (int) $page_id . ' and p.user_id=' . XT_USER_ID . ' order by sort_num asc';
	 ...
	 *
	 */
	function getPageModulesByPosition($page_id = null, $position = 0) {
		$db = $this->db;
		$sql = 'select position,isNative,param,p.id,p.title as newTitle,p.sort_num,p.in_use,c.* from ' . $db->getTable(T_PAGE_MANAGER) . ' as p' . ' join ' . $db->getTable(T_COMPONENTS) . ' as c' . ' on c.component_id=p.component_id' . ' where page_id=' . (int) $page_id . ' and p.position=' . $position . ' and p.user_id=' . XT_USER_ID . ' order by sort_num asc';

		$rs = $db->query($sql);

		return RST($this->groupByPos($rs));
	}
	...
	function getPagelistByType($type = '') {
		...
		$sql = 'SELECT * FROM ' . $table . ' WHERE `user_id`=' . XT_USER_ID;
		if ($type != '') {
		...
		$nativePageList = V('-:sys_pages');
		foreach ($nativePageList as $aPage) {
			$pageList[$aPage['page_id']] = $aPage;
		}
		return $pageList;
	}

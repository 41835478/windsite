...
if (!empty ($list)) {
	...
	foreach ($list as $row) {
		...
		$buttons = $isBack ? ((in_array($row['num_iid'], $nids) ? '<div class="ks-clear"><span class="followed-btn item-faved">已添加</span></div>' : ('<div class="ks-clear"><a class="addfollow-btn item-share" rel="e:addtb" data-nid="' . $row['num_iid'] . '" data-title="' . F('escape', str_replace(array (
			...
			'',
			''
		), $title)) . '" data-volume="' . $row['volume'] . '" data-cid="' . $row['cid'] . '" data-price="' . $row['price'] . '" data-nick="' . $row['nick'] . '" data-pic_url="' . $row['pic_url'] . '" href="#" style="color:red;"><span class="plus">+</span>添加该商品</a></div>'))) : ('<div class="ks-clear"><a class="addfollow-btn item-share" rel="e:tbs,i:' . $row['num_iid'] . ',p:' . $row['price'] . ',n:' . $row['nick'] . ',t:' . F('escape', $row['title']) . ',u:' . base64_encode($row['pic_url']) . '" href="#"><span class="plus">+</span>分享</a><a class="addfollow-btn item-fav" rel="e:tbf,i:' . $row['num_iid'] . ',n:' . $row['nick'] . '" href="#"><span class="plus">+</span>收藏</a></div>');

...
<title>兑换中心-淘客建站-我是淘客-新淘网</title>
</@ws.header>
<script>
$(function(){
$('#credits-tab').tabs();
...
<@xt.taoketemplate navselected='taoke' bdselected='site-credit'>
<@ws.info>
<span>
...
<TABLE class="wTable" width="100%" border="0" cellspacing="1" cellpadding="1">
	...
	<#if creditsHistory??&&(creditsHistory?size>0)>
		...
		<tr  class="<#if c_index%2==0>odd<#else>even</#if>"><td><#if c.c_type=="0">论坛积分兑换推广组<#else>家园积分兑换推广组</#if></td><td>${c.c_num}</td><td>${c.c_credits}</td><td>${c.updated}</td></tr>
		</#list>
		<#else>

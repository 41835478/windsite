//2011/06/17
var _e = function(){};
if(window.console) _e = function(str){console.info(str);};

(function(window, $){
    var sohuHD = window.sohuHD = {
        day : 24*60*60
        ,hour : 60*60
        ,sec : 60
        ,cache : {}
        ,timeFromNow : function(s){
        /* get 毫秒
         * return xx天前/xx小时前/xx分前/xx秒前/正在播放
         * */
            s = (new Date().getTime() - parseInt(s)) / 1000;
            var re = parseInt(s)+"秒前";
            if(s == 0){
                return '正在播放';
            }
            else{
                if(s >= this.day){
                    return Math.floor(s/this.day) + '天前';
                }
                else if(s >= this.hour){
                    return Math.floor(s/this.hour) + '小时前';
                }
                else if(s >= this.sec){
                    return Math.floor(s/this.sec) + '分前';
                }
            }
            return re;
        }
        ,transPlayTime:function(sec){
        /* get 毫秒
         * return X分X秒
         * */
			if(sec>0){
        ...
        ,eventProxy: function(box, event, op, type) {
/**
 * @public 批量绑定事件
 * @note 频繁的dom操作,比如事件绑定会减慢页面速度,
 *       对于一个交互操作比较集中的区域,建议用这个方法对整个父容器绑定一个代理方法
 *       对于内部html会被重写,需要重新绑定事件的页面元素,也推荐用这个方法
 * @param {DOM} box:绑定事件的父容器
 * @param {string} event:事件名称,如"click"
 * @param {object} op:包含对各个子元素触发事件的配置,有三个属性: hasAttr, hasId, hasClass
 * @param {int} * type如果为0,则不对回调函数做任何补充,如果缺少这个参数,会默认调用preventDefault
 * @return {object} 返回包含所有事件handler的对象,可以用于unbind
 * @example $("#parent").eventProxy({
 * 				hasAttr: {
 * 					"title": function(){
 * 						在这里this指向触发事件的元素
 * 					}
 ...
 * @TODO 提供解除事件绑定的方法
 */
            op = $.extend({ hasClass: {}, hasId: {}, hasTag: {}, hasAttr: {} }, op);
            $(box).bind(event, function(e){
                var t = e.target;
                var handler = op.hasId[t.id] || op.hasClass[t.className] || op.hasTag[t.nodeName.toLowerCase()] || null;
                var result = "nomatch";
                if(handler) {
    ...
    $.fn.eventProxy = function(event, op, type) {
        return this.each(function(){
            sohuHD.eventProxy(this, event, op, type);
        });
    };
    ...
    sohuHD.switchTab = function(tabs,opts){
        ...
        //boxs.hide();
        //tabs.eq(tabs.n).addClass(cssName);
        ...
        var go;
        if(auto){
            go=setInterval(autoNext, 5000);
            boxs.add(tabs).hover(
                function(){clearInterval(go);},
                function(){go=setInterval(autoNext, 5000);}
            );
        }
        
        var eventHandler = function(o){
            var n = o.n;
            o = $(o);
            if(cssReg.test(o.attr('class')))
                return;
            o.addClass(cssName);
            if(n != tabs.n)
                tabs.eq(tabs.n).removeClass(cssName);
            if(multiBox){
                boxs.eq(tabs.n).hide();
                boxs.eq(n).show();
            }
            ...
            o.blur();
            if(callback)
                callback(o,multiBox ? boxs.eq(n) : boxs);
        };

        tabs.each(function(i,o){
            $(o).removeClass(cssName);
            o.n = i;
            if(i == start) eventHandler(o);
        });
...
//压缩后需手动添加在尾部
sohuHD.isIE6 = false /*@cc_on || @_jscript_version < 5.7 @*/
sohuHD.isIE7 = false /*@cc_on || @_jscript_version == 5.7 @*/
sohuHD.gteIE7 = false /*@cc_on || @_jscript_version >= 5.7 @*/
sohuHD.isIE = /*@cc_on!@*/false

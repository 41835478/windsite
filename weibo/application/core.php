...
class APP {
	...
	function _initConfig() {
		...
		//TODO HTTP_X_FORWARDED_HOST 本地会返回两个代理地址，试试服务器再说
	...
	function getRequestRoute($is_acc = false) {
		...
		if (R_MODE == 2) {
			$ss = trim(V('S:PATH_INFO', ''), '/');
			if (empty ($ss)) {
				$m = $_SERVER['SERVER_NAME'] == 'www.xintaowang.com' ? 'map.index' : R_DEF_MOD;
			} else {
	...
	function addPreDoAction($doRoute, $type, $args = array (), $except = '') {
		APP :: setData($doRoute . ',' . $type, array (
			$doRoute,
			$type,
			$args,
			$except
	 ...
	 * @return 生成的URL
	 */
	function mkModuleUrl($mRoute, $qData = false, $entry = false, $R_MODE = '4') {
		...
		//--------------------------------------------------------------		
		if ($R_MODE == 0 || substr($mRoute, 0, 4) === 'mgr/') { //fixed by fxy060608 管理功能自动使用R_MODE==0
	...
	function V($vRoute, $def_v = NULL, $setVar = false) {
		...
		if ($_SERVER['SERVER_NAME'] == 'www.xintaotv.com' && $vRoute == '-:sysConfig/site_name') {
			return '新淘高清视频';
		}
		return $v[$vRoute];
	}
	...
	function F($fRoute) {
		...
		if (!$pp) {
			trigger_error("fRoute : [ $fRoute  ] is  invalid ", E_USER_ERROR);
			return false;
		}
	...
	function & O($oRoute) {
		...
		$oArr[$oRoute] = call_user_func_array(array (
			'APP',
			'_cls'
		), $p);
		return $oArr[$oRoute];
	...
	*/
	...
	function _getIncFile($fRoute, $type = 'cls', $ext = "") {
		...
		if ($ext === true && isset ($GLOBALS[V_GLOBAL_NAME]['MIX_TPL']) && in_array($type, array (
				'tpl',
				'pls',
				'mod'
			))) {
		...
		// 正常一般时运行，例如安装程序、后台等
		$f = array (
			'tpl' => P_TEMPLATE . "/" . $tpldir . $fp . EXT_TPL,
			...
			'top' => P_TOP . "/" . $fp . EXT_TOP, //增加TOP文件加载规则  fixed by fxy060608
	...
	function LOG($msg, $type = false) {
		if (strtolower(XWB_SERVER_ENV_TYPE) != 'common' || !LOG_LEVEL) {
			return false;
		}
	...
	function JSONP($rst, $errno = 0, $err = '', $callback = 'callback', $script = '') {
		echo "<script language='javascript'>{$callback}(" . json_encode(array (
			'rst' => $rst,
			'errno' => $errno * 1,
			'err' => $err
		)) . ");" . $script . "</script>";
...
class Xpipe {
	...
	*/
	function pagelet($r, $p = array (), $o = false, $isPerch = true) {
		if ($r == 'common.sideComponents') { //如果是侧边栏模块列表
			TPL :: module('xintao/pagelets', array (
				'p' => $p['type'],
				'position' => 2
			));
			return true;
		}
		if (XT_ISCRAWLER || $r == 'component/component_103.run' || $r == 'component/component_102.run' || $r == 'component/component_104.run') { //如果是蜘蛛或者102，103广告模块。fixed by fxy060608
			//标准输出 fixd by fxy060608
			...
			//echo $k,"-",$ost,"\n";
			$GLOBALS[V_GLOBAL_NAME]['PAGELETS'][$k] = array (
				$r,
				$p,
				$ki,
				$isPerch
	...
	*/
	function run() {
		if (!Xpipe :: usePipe() || XT_ISCRAWLER) { //如果是蜘蛛。fixed by fxy060608
			return false;
		}
	...
	*/
	...
	function _start() {
		...
		$iniCfg = array (
			...
			'sids' => XT_SIDS,
			'XT_SHOPS' => XT_SHOPS,
			'XT_IS_DIRECT' => XT_IS_DIRECT, //是否直接访问目标
	'XT_IS_SINGLE' => XT_IS_SINGLE,
			'XTTV_IS_MODULE' => USER :: get('isModule'), //是否是淘宝嵌入
	'XT_IS_MULTI' => XT_IS_MULTI,
			'XT_IS_TAOKE' => XT_IS_TAOKE,
			'XT_IS_VANCL' => XT_IS_VANCL,
			'XT_IS_WEIBO' => XT_IS_WEIBO,
			'XT_IS_LOGIN' => XT_IS_LOGIN,
			'XT_IS_POST' => XT_IS_POST,
			'PID' => XT_USER_PID,
			'SPID' => XT_USER_SPID,
			'XT_VANCL_NICK' => XT_VANCL_NICK,
			'XT_SITE_DOMAIN' => XT_SITE_DOMAIN,
			'taobaoNick' => USER :: get('taobao_nick'),
			'taobaoId' => USER :: taobaoId(), //TODO 其他平台标识
 ...
 * @return 			URL
 */
function URL($mRoute, $qData = false, $entry = false, $R_MODE = '4') {
	if ($mRoute == 'items' || $mRoute == 'products') { //如果是淘宝商品搜索，定制URL
		$qData = array (
			'search' => implode('-', array_values($qData = F('taobao.taobao_default_search', $qData)))
		);
	}
	elseif ($mRoute == 'vancls') {
		$qData = array (
			'search' => implode('-', array_values($qData = F('taobao.vancl_default_search', $qData)))
		);
	}
	elseif ($mRoute == 'posters') {
		$qData = array (
			'search' => implode('-', array_values($qData = F('taobao.poster_default_search', $qData)))
		);
	}
	elseif ($mRoute == 'tv.search' || $mRoute == 'video.search') {
		$qData = array (
			'search' => implode('-', array_values($qData = F('taobao.tv_default_search', $qData)))
		);
	}
	return APP :: mkModuleUrl($mRoute, $qData, $entry, $R_MODE);
}
/**
* 生成WAP兼容的URL页面链接
* 
...
class CACHE {
	...
	function tSet($gName, $id, $value, $ttl = 0) {
...
class dsMgr {
	...
	//------------------------------------------------------------------
	/**
	 * 调用一个数据交互方法、动作　dsMgr::tbCall($dsRoute, $opt); 同名快捷函数为 TB($dsRoute, $opt);
	 ...
	 */
	function tbCall($PRE = TB_CACHE_KEY_PRE, $eHandler, $dsRoute, $opt = false) {
				...
				if (is_numeric($cacheArr[0]) || $cacheArr[0] === '') {
					...
					switch ($flag[0]) {
						...
						case 'i' :
							...
							break;
						default :
							trigger_error("dsMgr cache OPT : [ $opt ] is  invalid ", E_USER_ERROR);
							exit;
		...
		if ($useCache && $cacheType != 'i') {
			...
			switch ($cacheType) {
				case 'g' :
					$rst = $gCache ? CACHE :: tGet($gCacheName, $gCacheId) : CACHE :: get($gCacheName);
					break;
					// 页面周期的缓存
				case 'p' :
			...
			if (is_array($rst) && isset ($rst['rst']) && $rst['rst'] !== null) {
				return $eHandler ? $rst['rst'] : $rst;
			}
		}
		//--------------------------------------------------------------
		...
		// 第三个参数开始将传递给数据调用组件
		$comRst = call_user_func_array(array (
			& $objArr[$stKey],
			$rData[3]
		), $arg);
		//--------------------------------------------------------------
		...
		if (!empty ($comRst['errno'])) {
			...
			elseif ($comRst['errno'] == '1040016' && !IS_IN_JS_REQUEST) {
				APP :: tips(array (
					'tpl' => 'error',
					'msg' => L('common__apiError__limitTip')
				));
			}
		...
		if ($useCache && $cacheType != 'i') {
			switch ($cacheType) {
				...
				case 'u' :
					$ttl = $ttl * 1;
					if ($gCache) {
						CACHE :: tSet($gCacheName, $gCacheId, $comRst, $ttl);
					} else {
	...
	function call($eHandler, $dsRoute, $opt = false) {
		...
		if ($useCache && $cacheType != 'i') {
			...
			switch ($cacheType) {
				case 'g' :
					$rst = $gCache ? CACHE :: gGet($gCacheName, $gCacheId) : CACHE :: get($gCacheName);
					break;
					// 页面周期的缓存
				case 'p' :
					$rst = APP :: getData($gCacheId, $gCacheName);
 ...
 */
function P_ERROR($msg = false, $component_id = -999, $title = '未知模块', $force = false) {
	//TODO P_ERROR调整为一直输出错误信息
...
/// 凡客数据交互组件的快捷访问方法 调用函数 dsMgr::tbCall， 返回标准返回值结构，可自行处理错误
function VANCLS() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_VANCLS_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
...
function VANCL() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_VANCL_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
}
/// 搜狐视频数据交互组件的快捷访问方法 调用函数 dsMgr::tbCall， 直接返回结果集
function TV() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_TV_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
}
/// 淘宝数据交互组件的快捷访问方法 调用函数 dsMgr::tbCall， 返回标准返回值结构，可自行处理错误
function TB() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
}
/// 淘宝单商品数据交互组件的快捷访问方法 调用函数 dsMgr::tbCall， 返回标准返回值结构，可自行处理错误
function ITEMS() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_ITEMS_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
...
function ITEM() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_ITEM_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
...
function SHOPS() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_SHOPS_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
...
function SHOP() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_SHOP_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
}
/// 淘宝画报数据交互组件的快捷访问方法 调用函数 dsMgr::tbCall， 返回标准返回值结构，可自行处理错误
function POSTERS() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_POSTERS_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
...
function POSTER() {
	$p = func_get_args();
	array_unshift($p, false);
	array_unshift($p, TB_CACHE_POSTER_KEY_PRE);
	return call_user_func_array(array (
		'dsMgr',
		'tbCall'
	), $p);
...
class USER {
	...
	/// 设置或者获取当前访问的 新淘高清视频ID，不转参数时获取，传参数时为读取
	function tvId() {
		$arg = func_get_args();
		if (!empty ($arg)) {
			USER :: set('__CLIENT_XTTV_ID', $arg[0]);
		} else {
			return USER :: get('__CLIENT_XTTV_ID');
		}
	}
	function isTvLogin() {
		$tvId = USER :: tvId();
		$tvNick = USER :: tvNick();
		if (!empty ($tvId) && !empty ($tvNick)) {
			return true;
		}
		return false;
	}
	...
	function tvNick() {
		$arg = func_get_args();
		if (!empty ($arg)) {
			USER :: set('__CLIENT_XTTV_NICK', $arg[0]);
		} else {
			return USER :: get('__CLIENT_XTTV_NICK');
		}
	}
	/// 设置或者获取当前访问的 新淘高清视频ID，不转参数时获取，传参数时为读取
	function tvSid() {
		$arg = func_get_args();
		if (!empty ($arg)) {
			USER :: set('__CLIENT_XTTV_SID', $arg[0]);
		} else {
			return USER :: get('__CLIENT_XTTV_SID');
		}
	}
	/// 设置或者获取当前访问的 新淘高清视频ID，不转参数时获取，传参数时为读取
	function tvPid() {
		$arg = func_get_args();
		if (!empty ($arg)) {
			USER :: set('__CLIENT_XTTV_PID', $arg[0]);
		} else {
			return USER :: get('__CLIENT_XTTV_PID');
		}
	}
	/// 设置或者获取当前访问的 新淘高清视频ID，不转参数时获取，传参数时为读取
	function tvTitle() {
		$arg = func_get_args();
		if (!empty ($arg)) {
			USER :: set('__CLIENT_XTTV_TITLE', $arg[0]);
		} else {
			return USER :: get('__CLIENT_XTTV_TITLE');
		}
	}
	/// 设置或者获取当前访问的 新淘高清视频ID，不转参数时获取，传参数时为读取
	function tvSession() {
		$arg = func_get_args();
		if (!empty ($arg)) {
			USER :: set('__CLIENT_XTTV_SESSION', $arg[0]);
		} else {
			return USER :: get('__CLIENT_XTTV_SESSION');
		}
	}
	...
	/// 当前访问者是否已登录,tid为空则未登录
	function isTBLogin() {
		$tid = USER :: get('__CLIENT_TB_USER_ID');
		return !empty ($tid);
	}
	///当前访问者是否已登录,tbid为空则未登录
	function isTaobaoLogin() {
		$tid = USER :: get('taobao_session');
		return !empty ($tid);
	}
	...
	/// 设置，或者获取taobaoId/用户ID；不传参数时获取，传参数时为读取
	function taobaoId() {
		$arg = func_get_args();
		if (!empty ($arg)) {
			USER :: set('__CLIENT_TAOBAO_USER_ID', $arg[0]);
		} else {
			return USER :: get('__CLIENT_TAOBAO_USER_ID');
		}
	}
	/// 重置会话存储
	function resetInfo() {

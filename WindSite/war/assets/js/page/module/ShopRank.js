 ...
 */
(function($) {
	$.widget("ui.pageShopRank", $.ui.pageModule, {
				/**
				 * 参数
				 */
				options : {
				...
				_initModule : function() {
					// PageModuleUtils.initShopRank(this.element);
				},
				/**
				 * 返回当前模块源码
				 */
 ...
 * 初始化模块内容编辑页面
 */
function initShopRankModuleConfig(func) {
 ...
 * @return {Boolean}
 */
function validateShopRankModuleConfig() {
	if (MODULE_ADTYPE != null) {
		switch (MODULE_ADTYPE) {
			case 'item' :
 ...
 *            func
 */
function createShopRankModuleContent(name, title, region, next, func) {
	var params = {};
	if (MODULE_ADTYPE != null) {
		switch (MODULE_ADTYPE) {
			case 'item' :
 ...
 *            func
 */
function updateShopRankModuleContent(id, title, func) {
	var params = {};
	if (MODULE_ADTYPE != null) {
		switch (MODULE_ADTYPE) {
			case 'item' :

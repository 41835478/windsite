	...
	function run() {
		...
		foreach (self :: $logList as $log_file => $aLog) {
			...
			if (isset ($_SERVER['REQUEST_URI'])) {
				$msg = "\r\n====================" . F('get_client_ip') . "==={$_SERVER['SERVER_NAME']}{$_SERVER['REQUEST_URI']} Start====================\r\n" . $msg .
	...
	function _getLogFile($type, $level) {
		...
		if (in_array($type, array (
			...
			if (LOG_LEVEL_INFO == $level) {
				return P_VAR . "/log" . date("/Y_m/Ymd") . '.' . LOG_LEVEL_INFO . ".$type.php";
			} else {
				return P_VAR . "/log" . date("/Y_W") . '.' . LOG_LEVEL_ERROR . ".$type.php";
			}
		}

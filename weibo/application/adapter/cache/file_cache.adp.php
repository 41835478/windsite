...
class file_cache {
	...
	function set($key, $value, $ttl = 0) {
		...
		$vData = array (
			$key => array (
				'data' => $value,
				'timeout' => (APP_LOCAL_TIMESTAMP + $ttl),
				'ttl' => $ttl
			)
		);
	...
	function _getSavePath($key) {
		...
		$d = '';
		if (strpos($key, TB_CACHE_ITEMS_KEY_PRE, 0) === 0) { //淘宝商品缓存
			$d = P_VAR_CACHE_TB_ITEMS . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_ITEM_KEY_PRE, 0) === 0) { //淘宝单商品缓存
			$d = P_VAR_CACHE_TB_ITEM . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_SHOPS_KEY_PRE, 0) === 0) { //淘宝店铺缓存
			$d = P_VAR_CACHE_TB_SHOPS . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_SHOPS_KEY_PRE, 0) === 0) { //淘宝单店铺缓存
			$d = P_VAR_CACHE_TB_SHOP . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_POSTERS_KEY_PRE, 0) === 0) { //淘宝画报缓存
			$d = P_VAR_CACHE_TB_POSTERS . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_POSTER_KEY_PRE, 0) === 0) { //淘宝单画报缓存
			$d = P_VAR_CACHE_TB_POSTER . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_VANCLS_KEY_PRE, 0) === 0) { //凡客缓存
			$d = P_VAR_CACHE_TB_VANCLS . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_VANCL_KEY_PRE, 0) === 0) { //凡客单商品缓存
			$d = P_VAR_CACHE_TB_VANCLS . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_TV_KEY_PRE, 0) === 0) { //搜狐视频缓存
			$d = P_VAR_CACHE_TB_TV . '/' . implode('/', $pArr);
		}
		elseif (strpos($key, TB_CACHE_KEY_PRE, 0) === 0) { //淘宝通用缓存（放在最后，以免前几个通匹配:已无此必要）
			$d = P_VAR_CACHE_TB . '/' . implode('/', $pArr);
		} else { //其他默认缓存

			...
			<div><a href="/"><img src="/assets/images/logo.png"></a></div></td><td valign=bottom>
			  <div class="wm_mac_nav" align=center>
			    <ul style="width:500px">
			      ...
			      <li><a href="/router/member/widget/my" class="current">我的设计<small>(${page.totalCount})</small></a></li>
...
<@ws.info>
	...
	您已设计 <strong style='color:#D02200;font-weight:bold;'>${page.totalCount}</strong> 个组件！
	</span>
	<span>
	您可以收藏的组件最高限额为
...
<input type="hidden" id="sortOrderHidden" value="${sortOrder}">
	<div class="search_sort_div">
      <ul class="search_sort_ul">
        <li	<#if sortOrder=="widgetUpdated_desc">class="current"</#if>><a href="/router/member/widget/my?q=${q}&pageNo=${page.pageNo}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=widgetUpdated_desc">更新时间排行</a></li>
        <li	<#if sortOrder=="favorite_desc">class="current"</#if>><a href="/router/member/widget/my?q=${q}&pageNo=${page.pageNo}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=favorite_desc"><i class="ssu_pj_i"></i>收藏排行</a></li>
        <li	<#if sortOrder=="used_desc">class="current"</#if>><a href="/router/member/widget/my?q=${q}&pageNo=${page.pageNo}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=used_desc"><i class="ssu_rq_i"></i>使用排行</a></li>
      </ul>
      <div class="wm_search">
	...
	<div class="wm_wrap_filters">
		...
		<div class="sort_pagina">
	    	<a href="<#if (page.pageNo<page.totalPageCount)>/router/member/widget/my?q=${q}&pageNo=${page.pageNo+1}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=${sortOrder}<#else>#</#if>" class="page_next <#if (page.pageNo>=page.totalPageCount)>page_next_no</#if>"></a>
			<span class="pagina_span">${page.pageNo}/${page.totalPageCount}</span>
			<a href="<#if (page.pageNo>1)>/router/member/widget/my?q=${q}&pageNo=${page.pageNo-1}&layout=${layoutfilter}&type=${typefilter}&cid=${cidfilter}&sortOrder=${sortOrder}<#else>#</#if>" class="page_pre <#if !(page.pageNo>1)>page_pre_no</#if>"></a>
		</div>
		<div class="sort_other" >按组件类型筛选:<select name="widgetTypes" id="mytypes">
			...
			<#if widgets??&&widgets?size!=0>
				...
				<#list widgets as w>
				 ...
				 <div class="editor-bar" align=center>
				 	...
				 	<li><a href="/router/member/designer/<#if w.widget.type.name=='blog'>blogdesigner<#elseif w.widget.type.name='html'>htmldesigner<#elseif w.widget.type.name='search'>searchdesigner<#else>widgetdesigner</#if>/update/${w.id}" target="_blank" class="w-modifywidget"></a></li>
				 	...
				 	<#if !(w.used??&&w.used!='0')><li><a href="#"class="w-deletewidget" cwid="${w.id}">删除</a></li></#if>
				 	</ul>
				 	<p style="line-height: 18px;text-align: center;display:block;margin-bottom:10px;">
				 	组件名称：<span class="k">${w.name}</span>　  　 　  　 
            适合投放：<#if w.cat??><a href="/router/member/widget/my?cid=${w.cat.cid}"><span class="k">${w.cat.name}</span></a></#if>　  　 
            布局:<a href="/router/member/widget/my?layout=${w.layout}"><span class="k"><@ws.layout w.layout></@ws.layout></span></a>　  　 
            作者：<a href="/router/member/widget/designers/${w.createdBy}">${w.nick}</a>
				...
				<#else>
				<div style="padding:20px; font-size:14px;text-align:center;">你当前没有设计过组件，到<a href="/router/member/widget/sys" style="font-size:14px;">组件模板</a>去挑一个开始设计吧。</div>
			</#if>
			<#assign url='/router/member/widget/my?q='+q+'&layout='+layoutfilter+'&cid='+cidfilter+'&sortOrder='+sortOrder+'&type='+typefilter>

...
if (!empty ($list)) {
	foreach ($list as $key => $row) {
		...
		echo '<tr><td>' . htmlspecialchars($row['weibo']) . '<br/><a href="' . $showUrl . '" target="_blank">新浪地址</a>' . $qqShowUrl . $shShowUrl . $wyShowUrl . '，<a href="' . $localShowUrl . '" target="_blank">本站地址</a></td><td>' . htmlspecialchars($row['nickname']) . '<br/><a href="' . $taUrl . '" target="_blank">新浪地址</a>，<a href="' . $localTaUrl . '" target="_blank">本站地址</a></td><td>' . date('Y-m-d H:i:s', $row['addtime']) . '</td></tr>';
	}
	echo '<tr><td colspan="2"><div class="pre-next">' . $pager . '</div></td></tr>';
} else {
	echo '<tr><td colspan=2>未找到商品自动营销记录</td></tr>';
}
?>
 ...
 * 
 * Since : July 2009 Date: ${date} Revision: ${revision}
 */
(function($) {
	...
	$.fn.gallery = function(opts) {
		...
		var links = this, overlay = api.getOverlay(), next = overlay
				.find(conf.next), prev = overlay.find(conf.prev), info = overlay
				.find(conf.info), progress = overlay.find(conf.progress), els = prev
				.add(next).add(info).css({
							opacity : conf.opacity
						}), close = api.getClosers(), index;
		...
		function load(el) {
			...
			image.onload = function() {
				...
				if (!img.length) {
					img = $("<img/>").attr("id", conf.imgId).css("visibility",
							"hidden");
					overlay.prepend($('<a id="a_' + conf.imgId + '" href="'
							+ el.attr('click_url') + '" target="_blank"></a>')
							.append(img));// FIXED 增加链接
				} else {
					$("#a_" + conf.imgId, overlay).attr('href',
							el.attr('click_url'));
				}
				var _title = el.attr('title');
				img.attr('alt', _title).attr('title', _title);
				...
				// activate trigger
				links.removeClass(conf.activeClass).eq(index)
				...
				// set info text & width
				var text = conf.template.replace("${title}",
						el.attr("title") || el.data("title")).replace(
						"${index}", index + 1)
						.replace("${total}", links.length)// 此处为商品增加链接与价格
						.replace("${click_url}", el.attr("click_url")).replace(
								"${price}", el.attr("price"));

				var padd = parseInt(info.css("paddingLeft"), 10)
						+ parseInt(info.css("paddingRight"), 10);
				info.html(text)// 注释掉宽度设定.css({width: width - padd});

	...
	$.widget("ui.customeWidgetEditor", {
		...
		_initShops : function() {
			...
			$('#custome_shops_list input[type="radio"]').click(function(event) {
				...
				self._saveItem({
							title : li.attr('t'),
							clickurl : '/tshop/' + li.attr('sid') + '.html',
							picurl : li.attr('pi'),
							cr : li.attr('cr'),
							sid : li.attr('sid'),
							nick : li.attr('nick'),
				...
				$('#custome_groups_items input[type="radio"]').click(
						function(event) {
							...
							self._saveItem({
								...
								picurl : picurl,
								price : li.attr('pr'),
								tclickurl : '/item/' + li.attr('wid') + '.html',
								co : li.attr('co'),
								nid : li.attr('nid'),
								nick : li.attr('nick'),
								level : "/assets/min/images/credit/"
			...
			$('#c_keyword_table .top_box li.key').hover(function() {
				...
				if ($(this).hasClass('selected')) {
					...
					var clickurl = '/keywords?words='
							...
							+ $('#c_keyword_cats').val();
					var tbc = 'http://search8.taobao.com/browse/search_auction.htm?q='

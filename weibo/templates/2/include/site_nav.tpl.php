<div class="site-nav">
	...
	<?php 
		...
		{
			//if(in_array($aNav['data']['page_id'],array(995,998,999,1000))){continue;}
			//TODO 处理非首页的nofollow
			...
			if ($pageId == '-1') {
				...
				$p = array();//array('page_id' => $pageId, 'navId' => $aNav['data']['id']);
			...
			if (21 == $id) { // 我的首页  ?>
				<h3>
					<a rel="nofollow" href="<?php echo $pageUrl; ?>" <?php echo $target;?> class="<?php echo $selected;?>"><s class="ico-home"></s><?php echo $aNav['data']['name']; ?></a>
				</h3>
				<ul>
					<li>
					<a rel="nofollow" href="<?php echo URL('index.profile');?>" class="<?php echo APP::getRequestRoute() == 'index.profile'?' cur':''?>"><?php LO('include__siteNav__myWeibo');?></a>
					</li>
					<li>
					<a rel="nofollow" href="<?php echo URL('index.comments');?>" class="<?php echo in_array(APP::getRequestRoute(), array('index.atme', 'index.comments', 'index.messages', 'index.notices'))?' cur':''?>"><?php LO('include__siteNav__myMessage');?></a>
						<a rel="nofollow" href="<?php echo URL('index.comments');?>" class="square hidden" id="referMe"><span><span id="t">8</span></span></a>
					</li>
					<li><a rel="nofollow" href="<?php echo URL('index.favorites');?>" class="<?php echo APP::getRequestRoute() == 'index.favorites'?' cur':''?>"><?php LO('include__siteNav__myFavs');?></a></li>
				</ul>
			<?php } else {
			...
			?>
				...
				<h3><a <?php echo (in_array($aNav['data']['page_id'],array(2,4,35,37),false)?'rel="nofollow"':'');?> href="<?php echo $pageUrl; ?>" <?php echo $target;?> class=" <?php echo $selected;?>"><s <?php echo $style; ?>></s><?php echo $aNav['data']['name']; ?></a></h3>

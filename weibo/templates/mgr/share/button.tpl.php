...
,'})()<','/script>'].join('');;
		...
		function replachScript(){
			...
			var ifshow=$('#ifShow');
			var ifsrc='<?php echo $shareurl?>?url=<?php echo urlencode(W_BASE_HTTP. W_BASE_URL); ?>&type='+type+'&count='+count+'&relateUid='+relateUid+'&appkey='+'<?php echo WB_AKEY; ?>'+'&title=<?php echo urlencode('演示将转发按钮放到'. F('escape',V('-:sysConfig/site_name')). '首页的效果，默认截取所在页面title'); ?>&rnd='+new Date().valueOf();

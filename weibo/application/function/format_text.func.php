		...
		foreach ($mc as $i=>$v){
			if ($i%2==1){
				if (substr($v, 0, 1).substr($v, -1, 1)=='##'){
					$newText.=' <a rel="nofollow" href="'.URL('search.weibo', array('k' => substr($v,1,-1))).'">'.htmlspecialchars($v).'</a> ';
				}elseif(substr($v, 0, 1)=='@'){
					$newText.=' <a rel="nofollow" href="'.URL('ta', array('name' => substr($v,1))).'">'.htmlspecialchars($v).'</a> ';
					...
					if (preg_match("#http://(t.sina.com.cn|weibo.com)/k/([^/]+)\$#sim",$ma[1],$mlink)){
						$newText.=' <a rel="nofollow" href="'.URL('search', array('k' => $mlink[2])).'">'.htmlspecialchars($ma[2]).'</a> ';
					}elseif(preg_match("#/pub/tags/([^/]+)\$#sim",$ma[1],$tag)){
						/// 标签链接处理
						$newText.=' <a rel="nofollow" href="'.URL('search.user',array('k'=>urldecode($tag[1]),'ut'=>'tags')).'">'.htmlspecialchars($ma[2]).'</a> ';
					}else{

...
class ad {
	...
	function save($data, $id = null) {
		...
		*/
		return RST($this->db->save($data, $id, T_AD, 'id'));
	}
	...
	function getAd($id = null) {
		$sql = 'SELECT * FROM ' . $this->table . ' WHERE user_id=' . XT_USER_ID;
		if ($id !== null) {
	...
	function getUsingAd($using = 1, $page = '') {
		$sql = 'SELECT *FROM ' . $this->table . ' WHERE `using`=' . $using . ' AND user_id=' . XT_USER_ID;
		$where = array ();
		if (!empty ($page)) {

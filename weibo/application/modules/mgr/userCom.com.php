...
class userCom {
	...
	/*
	 * 插入或更新一条用户登录数据(指定USER_ID)
	 * @param array() $data		用户数据数组
	 * @param int $id		sina_uid
	 * @return boolean
	 */
	function insertUserByUserId($data, $id = '', $USER_ID) {
		if (!is_array($data)) {
			return RST(false, $errno = 1210000, $err = 'Parameter can not be empty');
		}
	...
	function refreshUserTokens($USER_ID) {
		$this->_cleanCache();
		$db = APP :: ADP('db');
	...
	function counts($type = '') {
		$db = APP :: ADP('db');
		$sql = 'SELECT COUNT(*)AS count FROM ' . $db->getTable(T_USERS) . ' WHERE `user_id` = ' . XT_USER_ID; //fixed by fxy060608 增加userId过滤;
		if ($type === 'today') {
			$sql .= ' AND FROM_UNIXTIME(`first_login`,"%Y%m%d")="' . date('Ymd') . '"';
		}
		$count = $db->getOne($sql);
		return RST($count);
	}
	...
	*/
	...
	function getSinaUidByName($nickname) {
		if ($nickname = $db->escape($nickname)) {
			$db = APP :: ADP('db');
			$table = $db->getTable(T_USERS);
			$sql = "Select sina_uid From $table Where nickname='$nickname' And user_id=" . XT_USER_ID;
	...
	*/
	function getByUid($sina_uid) {
		...
		$where = " WHERE `sina_uid` = '" . $sina_uid . "' AND `user_id`=" . XT_USER_ID;
	...
	*/
	function getBySiteUid($site_uid) {
		...
		$where = " WHERE `uid` = '" . $site_uid . "' AND `user_id`=" . XT_USER_ID;
	...
	*/
	function getAllVerify($offset = 0, $each = 0) {
		...
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_USER_VERIFY . ' WHERE user_id=' . XT_USER_ID . ' ORDER BY `add_time` DESC ' . $limit;
		return RST($db->query($sql));
	}
	...
	function getVerify() {
		$db = APP :: ADP('db');
		$sql = 'SELECT * FROM ' . $db->getTable(T_USER_VERIFY) . ' WHERE user_id=' . XT_USER_ID;
		$rst = $db->query($sql);
		if ($rst === false) {
			return RST(false);
		}
		...
		for ($i = 0, $count = count($rst); $i < $count; $i++) {
			$data[(string) $rst[$i]['sina_uid']] = array (
				'nick' => $rst[$i]['nick'],
				'reason' => $rst[$i]['reason']
			);
		}
	...
	*/
	function getUseBanById($sina_uid) {
		...
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_USER_BAN . ' WHERE `sina_uid` = "' . $sina_uid . '" AND user_id=' . XT_USER_ID . ' ORDER BY `sina_uid` DESC';
		return RST($db->getRow($sql));
	}
	...
	*/
	function getVerifyById($sina_uid) {
		...
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_USER_VERIFY . ' WHERE `sina_uid` = "' . $keyword . '"  AND user_id=' . XT_USER_ID . ' ORDER BY `sina_uid` DESC ';
		if ($db->getOne($sql)) {
			return RST(true);
		} else {
			return RST(false);
	...
	*/
	function getBanByUid($sina_uid) {
		...
		$sql = 'SELECT * FROM ' . $db->getPrefix() . T_USER_BAN . ' WHERE `sina_uid` = ' . $keyword . '   AND user_id=' . XT_USER_ID . ' ORDER BY `sina_uid` DESC ';
		if ($db->getOne($sql)) {
			return RST(true);
		} else {
			return RST(false);
	...
	function saveBan($data, $id = '') {
		...
		$this->_cleanCache();
		return RST($db->save($data, $id, T_USER_BAN, 'id'));
	}
	...
	*/
	...
	function setUserAction($sina_uid, $action_type) {
		if (isset ($sina_uid) && in_array($action_type, array (
				1,
				2,
				3,
				4
			...
			$rst = $db->query(sprintf("select id from %s where sina_uid='%s' and user_id=" . XT_USER_ID, $table, $sina_uid));
			if (empty ($rst)) {
				$id = '';
			} else {
				$id = $rst[0]['id'];
	...
	function getUserActionList() {
		$db = APP :: ADP('db');
		$table = $db->getTable(T_USER_ACTION);
		$rst = $db->query(sprintf("select sina_uid,action_type from %s where user_id=" . XT_USER_ID, $table));
		return RST($rst);
	}
	...
	function getUserAction($sina_uid) {
		$db = APP :: ADP('db');
		$table = $db->getTable(T_USER_ACTION);
		$rst = $db->query(sprintf("select action_type from %s where sina_uid=%s and user_id=" . XT_USER_ID, $table, $sina_uid));
		if (empty ($rst)) {
			$rst = array ();
			$rst[0] = array ();
			$rst[0]['action_type'] = 4;
	...
	function getSinaFollowerTop($showNum) {
		$db = APP :: ADP('db');
		$table = $db->getTable(T_USERS);
		$sql = "Select sina_uid From $table Where user_id=" . XT_USER_ID . " Order By followers_count Desc Limit $showNum";
		$temp = $db->query($sql);
		$list = array ();

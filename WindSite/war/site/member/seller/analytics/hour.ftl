...
<title>时段分析-推广统计-我是卖家-新淘网</title>
...
$(function(){
	...
	$('#date').dateinput({lang: 'zh-CN', format: 'yyyy-mm-dd',max:'0 d',change: function() {
		...
		createHourProfile('chartProfile','${USER.sid}','${USER.nick}',isoDate);
	}});
	createHourProfile('chartProfile','${USER.sid}','${USER.nick}','${today}');
	$('input[type="radio"][name="date"]').change(function(){
		createHourProfile('chartProfile','${USER.sid}','${USER.nick}',$(this).val());
	});
});
</script>
<@xt.sellertemplate navselected='seller' bdselected='analytics-hour'>

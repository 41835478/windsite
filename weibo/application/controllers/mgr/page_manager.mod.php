...
class page_manager_mod extends action {
	...
	function set() {
		...
		APP :: redirect(URL('mgr/page_manager.setting', array (
			'id' => $page_id
		)), 3);
	}
	...
	function savesort() {
		...
		$result['single'] = $manager->setSort(explode(',', $single), $page_id, 3);
		APP :: ajaxRst($result, $result['main'] && $result['side'] && $result['single'] ? 0 : 1);
		exit;
	}
	...
	function setting() {
		...
		//TODO 后台验证服务权限
		if (!F('xintao.check_page', $page_id)) {
			echo '需订购指定服务';
			exit;
		}
		...
		*/
		...
		TPL :: assign('single_modules', isset ($modules[3]) ? $modules[3] : null);
		TPL :: assign('main_modules', isset ($modules[1]) ? $modules[1] : null);
		TPL :: assign('side_modules', isset ($modules[2]) ? $modules[2] : null);
		if ($page_id > 500) {
			if ($page_id == 801) { //影视搜索
				TPL :: assign('left_modules', isset ($modules[4]) ? $modules[4] : null);
				TPL :: assign('right_modules', isset ($modules[5]) ? $modules[5] : null);
				$this->_display('xintao/tv/search_setting');
			} else {
				$this->_display('xintao/page_setting');
			}
	...
	function createComponentView() {
		...
		*/
		//$data=
		if ($componentId == 97) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_items', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
		elseif ($componentId == 99) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_posters', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
		elseif ($componentId == 100) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_shopsSide', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
		elseif ($componentId == 101) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_items', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
		elseif ($componentId == 102 || $componentId == 103 || $componentId == 104) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_ads', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
		elseif ($componentId == 105) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_itemsSide', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
		elseif ($componentId == 106 || $componentId == 110) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_shop', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
		elseif ($componentId == 200) { //新淘网模块配置 fixed by fxy060608
			$editHtm = TPL :: plugin('mgr/xintao/component/xintao_pageManager_createComStep2_tvSearch', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		} else {
			$editHtm = TPL :: plugin('mgr/pageManager_createComStep2', array (
				'component_id' => $componentId,
				'page_id' => $page_id
			), FALSE, FALSE);
		}
	...
	function doCreateComponent() {
		...
		if (empty ($page_id) || empty ($data['component_id'])) {
			$this->_error('操作失败, 页面ID或 组件ID不能为空！', array (
				'default_action'
			));
		}
		...
		$db = APP :: ADP('db');
		$data['sort_num'] = $db->getOne('select max(sort_num) from ' . $db->getTable(T_PAGE_MANAGER) . ' where user_id=' . XT_USER_ID) + 1;
		...
		$url = URL('mgr/page_manager.setting', array (
			'id' => $page_id
		));
		// set db and return
	...
	function editComponentView() {
		...
		TPL :: assign('page', $page);
		TPL :: assign('page_id', $page_id);
		if ($data['component_id'] == 97) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_items');
		}
		elseif ($data['component_id'] == 99) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_posters');
		}
		elseif ($data['component_id'] == 100) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_shopsSide');
		}
		elseif ($data['component_id'] == 101) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_items');
		}
		elseif ($data['component_id'] == 102 || $data['component_id'] == 103 || $data['component_id'] == 104) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_ads');
		}
		elseif ($data['component_id'] == 105) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_itemsSide');
		}
		elseif ($data['component_id'] == 106 || $data['component_id'] == 110) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_shop');
		}
		elseif ($data['component_id'] == 200) { //新淘网模块配置 fixed by fxy060608
			$this->_display('xintao/component/xintao_pageManager_editComponent_tvSearch');
		} else {
	...
	function doEditComponent() {
		...
		// update and return
		$db = APP :: ADP('db');
		$result = $db->save($data, $pmId, T_PAGE_MANAGER, 'id');
		if ($result || $result === 0) {
	...
	*/
	...
	function delComponent() {
		...
		if ($pmId && $data = $db->get($pmId, T_PAGE_MANAGER, 'id')) {
			if (empty ($data['isNative'])) {
				// delete the page_manager record
				if ($db->delete($pmId, T_PAGE_MANAGER, 'id')) {
					$this->_succ('操作已成功', $url);
				}
			} else {
	...
	function doCreatePage() {
		...
		if ($pageId = $db->save($data, '', T_PAGES)) {
			...
			$this->_succ('操作已成功', URL('mgr/page_manager.setting', array (
				'id' => $pageId
			)));
		}
	...
	function doEditPage() {
		...
		if (!$pageId || !$data = $db->get($pageId, T_PAGES, 'page_id')) {
			$this->_error('找不到对应页面,请检查一下参数', array (
				'default_action'
			));
		}

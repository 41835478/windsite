...
class common_pls {
	/**
	 * 单栏Pagelet(无法处理实时输出的Pagelet（如广告联盟的js代码类型）)
	 */
	function singleComponents($p) {
		$param = array (
			'type' => 2
		);
	...
	function userSkin() {
		...
		$skinSortIndex = array (
			'0' => array (
				'name' => L('pls__common__userSkin__defaultTitle'),
				'skins' => array ()
			)
		);
		foreach ($skinSort['rst'] as $sort) {
			$skinSortIndex[$sort['style_id']] = array (
				'name' => $sort['style_name'],
				'skins' => array ()
			);
		}
		$realSkin = array ();
	...
	function subjectFollowed($sina_id) {
		...
		TPL :: module('subject_followed', array (
			'list' => $list
		));
	}
	...
	function xtSearchMod($mod) {
		TPL :: module('xintao/mod_search', $mod);
	}

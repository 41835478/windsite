...
class Nav
{
	...
	function getNavList($inUse=FALSE, $type='')
	{
		...
		$sql 	= "Select * From {$this->table} Where user_id=".XT_USER_ID." $inUse Order By parent_id, sort_num, id";
	...
	function getNavById($id)
	{
		...
		{
			$sql 	= "Select * From {$this->table} Where parent_id=".$this->db->escape($id)." and in_use=1 and user_id=".XT_USER_ID." Order By sort_num, id";
			$result = $this->db->query($sql);
	...
	function getNavByPageId($page_id){
		$sql = "SELECT * FROM {$this->table} WHERE page_id = '". (int)$page_id."'"." AND user_id=".XT_USER_ID;

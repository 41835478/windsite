...
class notice
{
	...
	*/
	function sendNotice($title, $content, $sina_uid = NULL, $screen_name = NULL, $sender_id = 0, $available_time = 0)
	{
		...
		foreach ($sina_uid as $uid) {
			...
			$values[] = "('$notice_id', '$uid','".XT_USER_ID."')";
		}
		
		$sql = "INSERT INTO `" . $this->tb_recipients . "` (`notice_id`, `recipient_id`,`user_id`) VALUES " . implode(',', $values);//fixed by fxy060608 增加userId过滤
	...
	*/
	function getNoticeNum($sina_uid = 0)
	{
		...
		$sql = "SELECT COUNT(*) FROM `" . $this->tb_notice . "` WHERE `user_id`=" . XT_USER_ID . ($sina_uid > 0 ? " AND `notice_id` IN (SELECT `notice_id` FROM `" . $this->tb_recipients . "` WHERE `recipient_id` = 0 OR `recipient_id` = '" . $this->db->escape($sina_uid) . "') $firstTimeSql AND `available_time` <= " . APP_LOCAL_TIMESTAMP . " AND `user_id`=" . XT_USER_ID : '');
	...
	*/
	...
	{
		$firstTimeSql = $this->_getUserNoticeStart($sina_uid);
		$sql = "SELECT * FROM `" . $this->tb_notice . "` WHERE `user_id`=" . XT_USER_ID . ($sina_uid > 0 ? " AND `notice_id` IN (SELECT `notice_id` FROM `" . $this->tb_recipients . "` WHERE `recipient_id` = 0 OR `recipient_id` = '" . $this->db->escape($sina_uid) . "') $firstTimeSql AND `available_time` <= " . APP_LOCAL_TIMESTAMP . " AND `user_id`=" . XT_USER_ID : '') . " ORDER BY `available_time` DESC LIMIT " . $offset . "," . $limit;
		$this->db->checksql_set(array('sub_select_allow'=>true));	
	...
	*/
	...
	{
		$sql = "SELECT `available_time` FROM `" . $this->tb_notice . "` WHERE `available_time` <= " . APP_LOCAL_TIMESTAMP . " AND `user_id`=" . XT_USER_ID . " ORDER BY `available_time` DESC LIMIT 1";
		return RST($this->db->getOne($sql));
	}
	...
	*/
	...
	{
		$sql = "SELECT a.*,b.`nickname` FROM `" . $this->tb_recipients . "` a LEFT JOIN `" . $this->db->getTable(T_USERS) . "` b ON a.`recipient_id`=b.`sina_uid` AND b.`user_id`=".XT_USER_ID." WHERE a.`notice_id`='" . $this->db->escape($notice_id) . "'";
		return RST($this->db->query($sql));
	}
	...
	*/
	function getUnreadNoticeNum($sina_uid, $maxNoticeTime)
	{
		...
		$sql = "SELECT COUNT(DISTINCT(`notice_id`)) FROM `" . $this->tb_recipients . "` WHERE (`recipient_id` = '" . $this->db->escape($sina_uid) . "' OR `recipient_id` = 0) AND `user_id`=" . XT_USER_ID . " AND `notice_id` IN (SELECT `notice_id` FROM " . $this->tb_notice . " WHERE `available_time` <= " . APP_LOCAL_TIMESTAMP . " AND `available_time` > '" . $this->db->escape($maxNoticeTime) . "')";
		$this->db->checksql_set(array('sub_select_allow'=>true));
		return RST($this->db->getOne($sql));
	}
	...
	*/
	function updateUserPreMaxTime($sina_uid, $max_time)
	{
		...
		//更新增加user_id过滤
		$sql = 'UPDATE ' . $this->db->getTable(T_USERS) . ' SET `max_notice_time`='.$max_time . ' WHERE `sina_uid` = '.$sina_uid.' AND `user_id` = '.XT_USER_ID;

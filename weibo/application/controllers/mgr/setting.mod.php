	...
	*/
	...
	'logo_link' => V('p:logo_link', '1') // Logo链接为我的首页或广场,1为广场,2为我的首页
			...
			F('xintao.update_config_file', array (
				'R_DEF_MOD' => V('p:def_mod'),
				'XT_DEF_SHARE_MOD' => V('p:def_share_mod'),
				'XT_DEF_SHARE_SUB' => V('p:def_share_sub')
			), XT_USER_ID); //更新def_mod
			//更新SESSION
			$rs = USER :: get('extra');
			$rs['def_mod'] = V('p:def_mod');
			$rs['def_share_mod'] = V('p:def_share_mod');
			$rs['def_share_sub'] = V('p:def_share_sub');
			$rs = USER :: set('extra', $rs);
			$this->_succ('已经成功保存你的配置', array (
				'editIndex'
			));
			exit;
		}
	...
	function editRewrite() {
		if ($this->_isPost()) {
			...
			if (V('p:rewrite_way', '0') && !$this->_checkApacheReweite()) {
				$this->_error('配置失败,Apache服务器没有开启rewrite模块', array (
					'editRewrite'
				));
			}
	...
	function skinBGUpload() {
		if ($this->_isPost()) {
			...
			while ($file && $file['tmp_name']) {
				...
				APP :: JSONP(array (
					'url' => F('fix_url', $skinBG['webpath']) . '?_rand=' . time()
				), 0, '', $callback, $script);
				return;
			}
	...
	function cacheClear() {
		if ($this->_isPost()) {
			...
			if (XWB_SERVER_ENV_TYPE !== 'sae') {
				...
				// 修改user_config.php中memcache前缀，以达到更新缓存目的(暂不提供memcache fixed by fxy060608)

        ...
        <div class="set-area">
        	<div class="form web-info-form">
            	<form action="" name="form1" method="post" id="this_form">
                    ...
                    <?php if(XT_IS_WEIBO == 'true' && (XT_SITE_DOMAIN != 't' . XT_USER_ID . '.xintaowang.com')){?>
                    <div class="form-row">
                        ...
                        <div class="form-cont">
                            ...
                            <p class="form-tips">（您域名的备案信息，将显示在页面底部）</p>
                        </div>
                    </div>
                    <?php }?>
                    <div class="form-row">
                        ...
                        <div class="form-cont">
                            ...
                            <p class="form-tips">配置第三方统计代码后，您可以登录该第三方统计查看站点访问情况</p>
                        </div>
                    </div>
...
<!--                    </div>-->
					<div class="form-row">
                        ...
                        <div class="form-cont">
                        	<select name="def_mod" class="select">
                        	<?php
$pageList = F('get_pages_select');
if ($pageList) {
	$rs = USER :: get('extra');
	$def_mod = $rs['def_mod'];
	foreach ($pageList as $page) {
		if ($page['check']) {
			if (!in_array($page['page_id'], V('-:uninclude_home'))) { //不配置我的首页|我的微博|活动首页|我的收藏（此类功能需登录）,画报详情|凡客详情|商品详情(单品类详情)
				echo '<option ' . ($def_mod == $page['url'] ? 'selected' : '') . ' value="' . $page['url'] . '">' . $page['page_name'] . '</option>';
			}
		} else {
			echo '<option ' . ($def_mod == $page['url'] ? 'selected' : '') . ' value="' . $page['url'] . '" disabled>' . $page['page_name'] . '--需订购</option>';
		}
...
?>
                        	...
                        	<p class="form-tips">访问您的站点时，默认访问该页面</p>
                    	</div>
                    </div>
                    <div class="form-row">
                        <label class="form-field" for="declare">分享首页设置</label>
                        <div class="form-cont">
                        	<?php 	
                        		$wowCats= V('-:wow');
                        		$rs = USER :: get('extra');
                        		$def_share_mod = $rs['def_share_mod'];
                        		$def_share_sub = $rs['def_share_sub'];
                        		$wowSubs=array();
                        	?>
	                        	<select name="def_share_mod" class="select" id="J_WowShareCats">
	<?php 
		foreach ($wowCats as $key=>$wowCat) {
			if($wowCat['name']==$def_share_mod){
				$wowSubs = $wowCat['sub'];
				echo '<option selected value="'.$wowCat['name'].'" data-value="'.$key.'">' . $wowCat['title'] . '</option>';
			}else{
				echo '<option value="'.$wowCat['name'].'" data-value="'.$key.'">' . $wowCat['title'] . '</option>';
			}
			
		}
	?>
                        		</select>
                        		<select name="def_share_sub" class="select" id="J_WowShareSubs">
    <?php 
		foreach ($wowSubs as $key=>$wowSub) {
			echo '<option '.($def_share_sub==$key?'selected':'').' value="'.$key.'">' . $wowSub['title'] . '</option>';
		}
	?>                		
                        		</select>
                        	<p class="form-tips">访问您站点的分享频道时，默认访问的分类内容</p>
                    	</div>
                    </div>
					<div class="form-row hidden">
                        <label class="form-field" for="declare">网站首页设置</label>
            ...
            <div class="form web-info-form">
                <div class="form-row">
                    ...
                    <div class="form-cont">
                            ...
                            <p class="form-tips" style="color:red;">请选择PNG格式的本地图片文件，文件大小不超过500KB，图片大小不超过200*65px</p>
                    </div>
                ...
                <div class="form-row logo_preview">
                    ...
                    <div class="form-cont">
                        <img id="logo_preview" <?php echo $config['logo'] ? ('src="'.F('fix_url', $config['logo']).'"') : '';?>" />
...
var WOW = <?php echo json_encode($wowCats);?>;
$(function(){
	$('#J_WowShareCats').change(function(){
		var key = $(this).find('option:selected').attr('data-value');
		var subs = WOW[key]['sub'];
		if(subs){
			$('#J_WowShareSubs').html('');
			for(subKey in subs){
				var sub = subs[subKey];
				$('#J_WowShareSubs').append('<option value="'+subKey+'">'+sub['title']+'</option>');
			}
		}
	});
})

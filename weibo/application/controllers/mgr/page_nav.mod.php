...
class page_nav_mod extends action {
	...
	function nav() {
		...
		$pages = F('get_pages_select');
		$pageList = array ();
		foreach ($pages as $key => $page) {
			$pageList[$page['page_id']] = $page;
		}
	...
	function delNav() {
		...
		if ($navId && $aNav = $db->get($navId, T_NAV, 'id')) {
			if ($aNav['isNative']) {
				$this->_error('温馨提示：不能删除系统预设组件', $url);
			}

			if ($db->delete($navId, T_NAV, 'id')) {
				$sql = 'DELETE FROM ' . $db->getTable(T_NAV) . ' WHERE parent_id=' . $navId . ' and user_id=' . XT_USER_ID;
				$rs = $db->execute($sql);
				if ($rs !== false) {
					$this->_succ('操作已成功', $url);
				}

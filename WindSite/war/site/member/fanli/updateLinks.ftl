...
<table width=100%><tr><td><label class="fm-label" for="sortOrder"><span class="required">*</span>顺序：</label><input type="text"  required="required" class="i-text" id="sortOrder" name="sortOrder" value="${link.sortOrder}" style="width:50px;"></td><td><label for="title" class="fm-label"><span class="required">*</span>标题：</label><input type="text"  required="required" minlength="3" maxlength="20" class="i-text" id="title" name="title" value="${link.title}"></td>
<td><label class="fm-label" for="url"><span class="required">*</span>地址：</label><input type="url"  required="required" class="i-text" id="url" name="url" value="<#if link.url?starts_with('/')>${'http://'+domain+link.url}<#else>${link.url}</#if>" style="width:300px;"></td>

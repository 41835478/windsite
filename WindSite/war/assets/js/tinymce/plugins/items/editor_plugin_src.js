...
(function() {
	...
	tinymce.PluginManager.requireLangPack('items');
	...
	tinymce.create('tinymce.plugins.itemsPlugin', {
				 ...
				 *            ed Editor instance that the plugin is initialized
				 *            in.
				 * @param {string}
				...
				init : function(ed, url) {
					...
					// tinyMCE.activeEditor.execCommand('mceitems');
					ed.addCommand('mceitems', function() {
								$('#itemsSelect').dialog('open');
							});
				...
				getInfo : function() {
					return {
						longname : 'items plugin',
						author : '冯晓云',
						authorurl : 'http://www.xintaonet.com',
						infourl : 'http://www.xintaonet.com',
						version : "1.0"
					};
				}
			});

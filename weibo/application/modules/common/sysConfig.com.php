...
class sysConfig {
	...
	function set() {
		...
		foreach ($params as $key => $value) {
			...
			$data[] = '("' . $db->escape($key) . '","' . $db->escape($value) . '","' . XT_USER_ID . '")';
		}

		$sql = 'REPLACE ' . $db->getTable(T_SYS_CONFIG) . '(`key`,`value`,`user_id`) VALUES' . implode(',', $data);
		$rst = $db->execute($sql);
		///删除缓存
		if ($rst) {
	...
	function get($key = null) {
		...
		$row = $db->query('SELECT * FROM ' . $db->getTable(T_SYS_CONFIG) . ' WHERE user_id = ' . XT_USER_ID); //fixed by fxy060608 增加userId过滤;;

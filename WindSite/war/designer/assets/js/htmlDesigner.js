...
function deployCustomeWidget(wid, type) {
	...
	if (validateCustomeWidget()) {
		...
		var tbClone = $('#widget-customer div:first').clone();
		var content = '';// 其他推广
		var tbContent = '';// 淘宝推广
		try {
			$('a', cwClone).each(function() {// 转换链接
						var href = $(this).attr('href');
						href = href.replace('http://www.xintaonet.com', '');// 移除新淘网默认网址
						$(this).attr('href', '${siteurl}' + href);
					});
			content = cwClone.html();
			$('a', tbClone).each(function() {// 转换链接
						var href = $(this).attr('href');
						$(this).attr('href', $(this).attr('tbc'))
								.removeAttr('tbc');// 转换为淘宝推广链接
					});
			tbContent = tbClone.html();
		} catch (e) {
			alert(e);
			return false;
		}
		$('#deployWidget,#deployUpdateWidget').button('disable');
		var sender = new WindSender("/router/member/designer/htmldesigner/widget/"
				+ type + "/" + wid);
		sender.load("POST", {
					"title" : title,
					"cid" : cid,
					...
					if (response.isSuccess()) {
						...
						if ('create' == type) {
							document.location.href = '/router/member/designer/htmldesigner/update/'
									+ response.body.id;
						}

	...
	function Posters($elem, options) {
		...
		$.extend(self, {
			...
			displaySpecifiedImage : function(index) {
				...
				if (posterImage[0].complete || posterImage[0].readyState === 4) {
			...
			updateMarkers : function(thumb) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					var text = encodeURIComponent(item.markerItemTitle + '，￥：'
							+ item.markerItemPrice + '，详情：http://'
			...
			_convertLink : function(item) {
				if (PID && item.markerItemLink) {
					if (item.markerItemLink.indexOf('item.taobao.com') != -1) {// 非推广链接
						return o.defaultLink.replace('NUMIID',
								item.markerItemId)
					} else {
						return item.markerItemLink.replace(
								/mm_\d{0,24}_\d{0,24}_\d{0,24}/gi, PID)
								.split('#')[0]
								+ '&pid=' + PID;
					}

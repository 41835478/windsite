 ...
 */
(function($) {
	$.widget("ui.pageShopDisplay", $.ui.pageModule, {
				/**
				 * 参数
				 */
				options : {
 ...
 * 初始化模块内容编辑页面
 */
function initShopDisplayModuleConfig(func) {
 ...
 * @return {Boolean}
 */
function validateShopDisplayModuleConfig() {
	if (MODULE_ADTYPE != null) {
		switch (MODULE_ADTYPE) {
			case 'item' :
 ...
 *            func
 */
function createShopDisplayModuleContent(name, title, region, next, func) {
	var params = {};
	if (MODULE_ADTYPE != null) {
		switch (MODULE_ADTYPE) {
			case 'item' :
 ...
 *            func
 */
function updateShopDisplayModuleContent(id, title, func) {
	var params = {};
	if (MODULE_ADTYPE != null) {
		switch (MODULE_ADTYPE) {
			case 'item' :

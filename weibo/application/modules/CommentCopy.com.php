...
class CommentCopy
{
	...
	function counts($type = '') {
		$sql = 'SELECT COUNT(*)AS count FROM ' . $this->db->getTable(T_COMMENT_COPY).' WHERE `user_id` = '.XT_USER_ID;//fixed by fxy060608 增加userId过滤;
		if ($type === 'today') {
			$sql .= ' AND FROM_UNIXTIME(`dateline`,"%Y%m%d")="'. date('Ymd') . '"';
		}

...
class topics {
	...
	function saveSort($cty, $ids) {
		...
		$ids = implode(',', $ids);
		$sql = 'UPDATE ' . $this->table_today_topic . ' SET sort_num=FIND_IN_SET(id, "' . $ids . '") WHERE topic_id=' . $cty . ' and user_id='.XT_USER_ID;
	...
	function addApp($cty_id, $app_id) {
		...
		$sql = 'SELECT app_with FROM ' . $this->table_cty . ' WHERE topic_id=' . $cty_id .' and user_id='.XT_USER_ID;
	...
	function delCategory($id) {
		...
		$sql = 'SELECT native FROM ' . $this->table_cty . ' WHERE topic_id=' . $id .' and user_id='.XT_USER_ID;
	...
	function getTopicByCty($cty_id, $each=20, $offset=0) {
		...
		$sql = 'SELECT * FROM ' . $this->table_today_topic . ' WHERE topic_id=' . $cty_id . ' AND user_id='.XT_USER_ID.' ORDER BY sort_num ASC,ext1 DESC,date_time DESC LIMIT ' . $offset . ',' . $each;
		$rst = $this->db->query($sql);
		if ($rst !== false) {
			$this->count_sql = 'SELECT COUNT(*) FROM ' . $this->table_today_topic . ' WHERE topic_id=' . $cty_id .' AND user_id='.XT_USER_ID;
		}
		return RST($rst);
	}
	...
	function countTopic($cty_id) {
		$rs = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->table_today_topic . ' WHERE topic_id=' . $cty_id . ' AND user_id='.XT_USER_ID);
		return RST($rs);
	}
	...
	function hasTopics($category_id) {
		$select_count = 'SELECT COUNT(*) FROM ' . $this->table_today_topic . ' WHERE topic_id=' . $category_id . ' and user_id='.XT_USER_ID;

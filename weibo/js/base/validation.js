(function() {
	...
	/**
	 * @class Xwb.ax.ElementValidationContext 这是个接口类，接口方法为{@link Xwb.ax.ElememtValidatorChain}类所用，
	 *        可以提供满足验证单个元素所需的所有数据。<br/> Xwb已有一个该接口的实现类{@link Xwb.ax.ValidationMgr}，通常情况下无需关注此接口，
	 *        开发中更不会直接应用本接口。 该接口主要的作用有：
	 *        <ul>
	 *        <li>提供验证元素的验证器</li>
	 *        <li>验证成功或失败报告入口</li>
	 ...
	 * @param {HtmlElement}
	 *            element
	 * @param {Xwb.ax.ElememtValidatorChain}
	 *            elementChain
	 * @method report
	 */
	...
	/**
	 * @class Xwb.ax.ElememtValidatorChain 本类利用接口{@link Xwb.ax.ElementValidationContext}提供的方法与数据验证单个元素。<br/>
	...
	ElememtValidatorChain.prototype = {
		/**
		 * @property error 错误次数，可在callback调用时检测当前验证后的错误次数
		 * @type Number
		 */

		// error : 0,
		/**
		 * @property validating 是否验证中
		 * @type Boolean
		 */
		 ...
		 * 
		 * @param {Boolean}
		 *            callback 验证结束后回调， callback调用时this指向当前{@link Xwb.ax.ElememtValidatorChain}实例
		 */
		...
		doPrehandling : function() {
			...
			while ((cmd = ctx.nextValidatorData(idx++, this.elem))
					&& cmd.isPreCmd) {
				if (typeof cmd[0] === 'string') {
					var vd = ctx.getValidator(cmd[0]);
					if (!vd)
						throw 'ValidationException:Undefined validator \''
								+ cmd[0] + '\' in element ' + this.elem.name;
					vd.call(this, this.elem, cmd[1]);
				} else
					cmd[0].call(this, this.elem, cmd[1]);
			}
		},
			...
			if (vds) {
				...
				if (vds.isPreCmd) {
					...
					if (typeof vds === 'function') {
						// 寄存数据
						vds[1] = {};
						vds.call(this, this.elem, this.getValue(), vds[1],
								this.nextChain);
					} else {
						// 验证器标识字符串
						// 结构为[sid, data]
						...
						if (typeof vds[0] === 'string') {
							...
							if (!vd)
								throw 'ValidationException:Undefined validator ['
										...
										+ '] in element '
										+ this.elem.name;
							vd.call(this, this.elem, this.getValue(), vds[1],
									this.nextChain);
						} else
							vds[0].call(this, this.elem, this.getValue(),
									vds[1], this.nextChain);
					}
				}
			} else {
				if (!this.error) {
					if (__debug)
						console.log('onpass', this.elem, this);
	 ...
	 * @extends Xwb.ax.ElementValidationContext 表单验证类，对表单中所有需要验证的元素执行验证处理。<br/>
	 *          本类提供与具体应用无关的方式验证常用的表单数据的有效性。<br/> 支持元素同步与异步验证。<br/>
	 ...
	 *          什么是验证器？<br/> 验证器其实就是一个回调函数，除了库提供一套通用的验证器外，也可自行注册验证器。<br/>
	 *          每个验证器都应用一个标识，通过{@link #reg}方法注册验证器。<br/> 验证器标识字符串被HTML模板的表单元素<b>vrel</b>属性引用，指明验证该元素所需的验证器。<br/>
	 *          如果在vrel属性里的验证器标识加下划线，那么此时的验证器就是一个预处理器，
	 *          在表单验证管理器初始化后就会执行，而且只会执行一次。<br/> 验证器rel属性数据格式是什么？<br/>
	 ...
	 *          如果rel数据有HTML保留字符，请先用&xx;转义，比如双引号。 属性内容由验证器自己定义。<br/> 例：
	 * 
	 * <pre><code>
	 * 	 html:
	...
	X.ax.ValidationMgr.prototype = {
		...
		/**
		 * 注册表单元素验证器。<br/> 验证器调用格式：<br/> validator(element, value, data,
		 * next);<br/> element:当前验证元素<br/> value:当前验证元素值<br/> data:当前验证器rel数据<br/>
		 * next:验证结束后的调用函数 ，传递下一个验证<br/> 注意：验证元素值前除了空检测外其它情况都应该认为空值是可以<b>通过</b>的。
		 * 
		 * @param {String}
		 *            name 验证器名称
		 * @param {Function}
		...
		reg : function(cmd, validator) {
			...
			if ($.isArray(cmd)) {
				...
				validators[cmd] = validator;
			return this;
		},

		/**
		 * @cfg {Function} onsuccess 验证结束后回调方法，调用为 onsuccess(formData,
		 *      finalChain)，
		 *      formData为表单元素经过验证处理后的数据；finalChain为onsuccess必须回调的方法，以正常结束表单的验证。
		 *      如果是异步提交，可以在该方法内提交表单。<br/> 方法返回false可取消浏览器默认的表单提交。
		 */
		...
		_doNextChain : function() {
			...
			if (this.currElIdx === len) {
				if (!this.error) {
					if (__debug)
						console.log('onsuccess', this.data);
					// 返回非false进行表单form提交
				...
				if (el.disabled)
					...
					this._validateElem(el, this.nextChain, true);
			}
		},

		_validateElem : function(el, callback, multival) {
			// fixed by fxy060608,添加form-row的隐藏不校验逻辑
			var elem = $(el);
			if (elem.attr('vrel')) {
				var row = elem.parents('.form-row:first');
				if (row.length == 1
						&& elem.parents('.form-row:first').is(':hidden')) {// 如果存在行，且隐藏
					callback && callback();
				} else {
		 ...
		 * 为数组时，第一个为指令字符串，第二个为指令数据map结构。 格式为 cmd=k:v,k2:v2|cmd2=k:v,k2:v2 ...
		...
		tipWarn : function(elem, msg) {
			...
			if (tipId) {
				var jqTip = this.getWarnTip(elem, tipId);
				var jqTxt = jqTip.cssDisplay(true).find(this.tipTextNode);
				if (!jqTxt.length)
					jqTxt = jqTip;

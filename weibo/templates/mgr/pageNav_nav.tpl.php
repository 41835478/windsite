...
function getSelectHtml($id, $pageId, & $pageList) {
	...
	$uninclude = V('-:uninclude_nav');
	foreach ($pageList as $aPage) {
		if (in_array($aPage['page_id'], $uninclude))
			continue;
		$selected = ($pageId == $aPage['page_id']) ? 'selected' : '';
		if ($aPage['check']) {
			$pageSelect .= "<option value='{$aPage['page_id']}' $selected >{$aPage['page_name']}</option>";
		} else {
			$pageSelect .= "<option value='{$aPage['page_id']}' $selected disabled>{$aPage['page_name']}--需订购</option>";
		}

	}

	$selected = ($pageId === '-1') ? 'selected' : '';
	$pageSelect .= "<optgroup label='--------------------------'><option value='-1' $selected >自定义链接</option></optgroup></select>";
...
function aNavHtml($data, & $pageList, $isSon = FALSE) {
	...
	if ($data['page_id'] && isset ($pageList[$data['page_id']])) {
		$pageId = $pageList[$data['page_id']]['native'] ? FALSE : array (
			'page_id' => $data['page_id']
		);
		$pageName = $pageList[$data['page_id']]['page_name'];
		$url = URL($pageList[$data['page_id']]['url'], $pageId, 'index.php', 2);

		/*if (!in_array($data['page_id'], array (
				3,
				6,
				35,
				7,
			...
			$editLink = '<a href="' . URL('mgr/page_manager.setting', array (
				'id' => $data['page_id']
			)) . '" class="icon-set">设置</a>';
		}
	...
	$replace['#delLink#'] = '<a href="javascript:delConfirm(\'' . URL('mgr/page_nav.delNav', array (
		'id' => $data['id']
	)) . '\')" class="icon-del">删除</a>';
...
foreach ($pageList as $key => $aPage) {
	$pageId = $aPage['native'] ? FALSE : array (
		'page_id' => $key
	);
	$url = URL($aPage['url'], $pageId, 'index.php', 2);
	echo "url_$key:'$url',";
}
?>
	        url_0:''
...
foreach ($pageList as $aPage) {
	...
	if ($aPage['check']) {
		...
		echo "'<option value=\"{$aPage['page_id']}\" disabled>{$aPage['page_name']}--需订购</option>',";
	}
	//echo "'<option value=\"{$aPage['page_id']}\" >{$aPage['page_name']}</option>',";
}
echo "'<optgroup label=\"--------------------------\"><option value=\"-1\" >自定义链接</option></optgroup>',";
?>
				'			</select>',
